...
def main() -> int:
    ...
    if control_count != 32 or test_count != 9:
        errors.append(
            f"expected 32 controls and 9 shared tests, found {control_count} and {test_count}"
        )
    ...
    if repeated:
        errors.append(f"mass-repeated acceptance templates detected: {sorted(repeated.values(), reverse=True)[:5]}")
