...
for entry in file_entries:
    ...
    missing_fields = sorted(
        field for field in required_product_fields if not entry.get(field)
    )
    if missing_fields:
        errors.append(
            f"File inventory entry {entry.get('path')} is missing: {', '.join(missing_fields)}"
        )
...
if missing_guardrails:
    errors.append(
        "Missing inherited infrastructure guardrails: "
...
if not isinstance(modules, list) or len(modules) < 17:
    errors.append(
        "Legacy module inventory must retain the complete ConstruControl coexistence surface"
    )
...
for module in modules:
    ...
    if module.get("shell_required") is not True:
        errors.append(
            f"Legacy module is not covered by the unified shell contract: {code}"
        )
...
unregistered_workspace_targets = sorted(
    set(unregistered_workspace_targets) - allowed_secondary_targets
)
if unregistered_workspace_targets:
    errors.append(
        "Legacy workspace targets missing coexistence ownership: "
        + ", ".join(unregistered_workspace_targets)
    )
...
for phrase in (
    ...
    if phrase not in legacy_target_text:
        errors.append(
            f"Legacy target architecture is missing coexistence contract: {phrase}"
        )
...
if ownership_counts.get("ConstruControl legacy", 0) <= 0:
    errors.append(
        "Canonical inventory does not preserve the legacy ConstruControl boundary"
    )
