...
for path in (AUDIT, TARGET, INVENTORY, FILE_INVENTORY, WORKSPACE):
	if not path.is_file():
...
for entry in file_entries:
	if not str(entry.get("ownership", "")).startswith("ConstruControl"):
		continue
...
if unregistered_workspace_targets:
	errors.append(
		"Workspace targets missing architecture ownership: " + ", ".join(unregistered_workspace_targets)
	)
