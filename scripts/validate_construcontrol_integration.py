...
if (
	len(assets.get("pages", [])) < 2
	or len(assets.get("reports", [])) < 3
	or len(assets.get("print_formats", [])) < 3
):
	errors.append("Runtime pages, reports or print formats are incomplete")
workspace = json.loads(
	(cc / "workspace" / "construcontrol" / "construcontrol.json").read_text(encoding="utf-8")
)
...
report = {
	"ok": not errors,
	"functional_doctypes": len(definitions),
	"required_present": not missing,
	"errors": errors,
	"images_imported_policy": 0,
}
