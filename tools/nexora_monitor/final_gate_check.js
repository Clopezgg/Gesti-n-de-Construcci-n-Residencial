...
const [matrixText, results, defects, review] = await Promise.all([
  text(paths.matrix), json(paths.results), json(paths.defects), text(paths.review),
]);
const audit = validateAuditResults(matrixText, results, defects);
failures.push(...audit.errors);
if (!audit.model.gateReady) failures.push("audit_cli gate no esta listo: existen validaciones o defectos pendientes.");
...
if (head && !review.includes(head)) failures.push("FINAL_REVIEW_PACKAGE.md no contiene el HEAD exacto actual.");

let runs = [];
try {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), 12000);
  const response = await fetch(`https://api.github.com/repos/${REPO}/actions/runs?head_sha=${encodeURIComponent(head)}&per_page=100`, {
...
const gate = {
  ...
  workflows: runs.map((run) => ({ name: run.name, run_id: run.id, status: run.status, conclusion: run.conclusion, head_sha: run.head_sha })),
