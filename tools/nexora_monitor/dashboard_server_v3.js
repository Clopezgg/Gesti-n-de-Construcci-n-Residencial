...
async function consoleState() {
  ...
  const latestFailure = [...recent].reverse().find((item) => ["technical_error", "objective_mismatch"].includes(item.level)) || null;
...
let statusCache = {
  ...
  safety: { targetBranch: BRANCH, pullRequest: PR_NUMBER, mergeAllowed: false, tagAllowed: false, deploymentAllowed: false },
};
...
async function refreshStatus() {
  ...
  try {
    const [matrixText, auditResults, defects, live, session, console] = await Promise.all([
      ...
      readJson(PATHS.auditResults, { schema_version: 1, status: "active", active_block: 0, requirements: {} }),
      readJson(PATHS.defects, { schema_version: 1, defects: [] }),
      readJson(PATHS.live, { agent_status: "idle", phase: "Sin datos", current_block: 0, task: "Esperando auditoría", detail: "", tests: [], events: [], blocking_issue: null }),
    ...
    const reviewReady = audit.gateReady
      && allWorkflowsGreen
      && packageExists
    ...
    if (branch !== BRANCH || githubSummary.technical_error > 0 || audit.metrics.technical_error > 0 || session.status === "failed") {
      ...
      systemLevel = "blocked";
    } else if (session.status === "running" || githubSummary.running > 0 || live.agent_status === "working" || audit.metrics.running > 0) {
      systemLevel = "running";
    } else if (reviewReady) {
...
console.log("La matriz ya no certifica por sí sola: manda AUDIT_RESULTS.json + DEFECTS.json + CI del mismo SHA.");
