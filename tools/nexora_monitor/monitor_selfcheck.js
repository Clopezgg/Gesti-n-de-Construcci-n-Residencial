...
async function text(path) {
  ...
  catch (error) { errors.push(`No se pudo leer ${path}: ${error.message}`); return ""; }
}
async function json(path) {
  const raw = await text(path);
  try { return JSON.parse(raw); }
  catch (error) { errors.push(`JSON invalido ${path}: ${error.message}`); return {}; }
}

const [matrixText, results, defects, live, html] = await Promise.all([
  text(paths.matrix), json(paths.results), json(paths.defects), json(paths.live), text(paths.html),
]);
...
if (auditValidation.model.matrix.rowsFound !== 166) errors.push(`La matriz contiene ${auditValidation.model.matrix.rowsFound} requisitos, no 166.`);

const requiredIds = [
  "auditBar", "certBar", "metrics", "nodes", "blockTitle", "greenList", "orangeList", "redList",
  "defects", "currentTask", "activity", "requirements", "actions", "gate", "gateMessage",
];
for (const id of requiredIds) {
  if (!html.includes(`id="${id}"`)) errors.push(`dashboard.html no contiene #${id}.`);
}
const scriptMatches = [...html.matchAll(/<script>([\s\S]*?)<\/script>/gi)];
if (scriptMatches.length !== 1) errors.push(`dashboard.html debe contener un script principal; encontrados ${scriptMatches.length}.`);
else {
  try { new Function(scriptMatches[0][1]); }
  catch (error) { errors.push(`JavaScript del dashboard invalido: ${error.message}`); }
}
