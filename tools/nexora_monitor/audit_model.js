...
export function validationProfile(requirement) {
  ...
  if (FINANCIAL_PREFIXES.has(prefix) || /saldo|financ|contab|presup|pago|costo|inventario/.test(text)) {
    validations.push(validation("financial_integrity", "Integridad financiera y conciliación", "integrity"));
  }
...
export function buildAuditModel(matrixText, rawResults = {}, rawDefects = {}) {
  ...
  const resultMap = rawResults?.requirements && typeof rawResults.requirements === "object"
  ...
  const requirementModels = requirements.map((requirement) => {
    ...
    const relatedDefects = defects.filter((item) => item.requirementId === requirement.id && item.status !== "resolved");
    ...
    return {
      ...
      implementationFiles: Array.isArray(rawRequirement?.implementation_files) ? rawRequirement.implementation_files : [],
      validations,
      validationSummary,
  ...
  for (let number = 0; number <= 20; number += 1) {
    ...
    const validationStatuses = blockRequirements.flatMap((item) => item.validations.map((validation) => validation.status));
  ...
  const allValidationStatuses = requirementModels.flatMap((item) => item.validations.map((validation) => validation.status));
  ...
  const activeBlock = Number.isInteger(activeBlockCandidate) && activeBlockCandidate >= 0 && activeBlockCandidate <= 20
  ...
  const gateReady = requirements.length === 166
...
export function validateAuditResults(matrixText, results, defects) {
  ...
  const errors = [];
  if (model.matrix.rowsFound !== 166) errors.push(`Se esperaban 166 requisitos y se encontraron ${model.matrix.rowsFound}.`);
  for (const requirement of model.requirements) {
    for (const validationItem of requirement.validations) {
      if (validationItem.status === "certified") {
        if (!validationItem.detail) errors.push(`${requirement.id}/${validationItem.id}: falta detalle.`);
        if (!validationItem.sha || !/^[0-9a-f]{40}$/i.test(validationItem.sha)) {
          errors.push(`${requirement.id}/${validationItem.id}: falta SHA completo de 40 caracteres.`);
        }
        if (!validationItem.evidence.length) errors.push(`${requirement.id}/${validationItem.id}: falta evidencia.`);
      }
    }
  }
