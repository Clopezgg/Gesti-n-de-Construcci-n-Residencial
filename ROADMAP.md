...
| 1 · Experiencia | **Bloque D — sistema de diseño completo** | El panel (`nexora_dashboard.js`) ya usa `nxr-ds-btn` en sus once botones, con un tamaño compacto nuevo (`--sm`) para las acciones de cabecera de tarjeta. Quedan nueve pantallas con `btn btn-xs`/`btn-default` y las tablas del marco (`table`, `table-bordered`); no existe todavía un componente `nxr-ds-table` |
