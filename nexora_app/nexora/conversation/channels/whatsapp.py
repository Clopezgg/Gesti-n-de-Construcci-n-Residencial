...
def _record_sent_message(external_message_id: str, recipient: str, correlation_id: str) -> None:
	...
	try:
		...
		# `external_message_id` es único: si ya existe, este mismo envío ya
		# quedó registrado (p. ej. una respuesta de Meta perdida en la red hizo
		# que `_open_graph_request` reintentara una llamada que sí había
		# tenido éxito). Meta ya recibió y procesó el mensaje real — no hay
		# nada que registrar de nuevo ni que fabricar como error.
		return
	except Exception:  # noqa: BLE001 -- el mensaje ya se envió de verdad a Meta; que falle
		# únicamente su registro interno de seguimiento no debe hacer fallar el
		# envío para quien lo esperaba del otro lado, mismo criterio que
		# notifications/service.py.
		frappe.log_error(
			message=f"No se pudo registrar el mensaje WhatsApp saliente {external_message_id} "
