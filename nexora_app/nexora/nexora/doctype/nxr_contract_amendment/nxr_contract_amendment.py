...
class NXRContractAmendment(Document):
	...
	def validate(self) -> None:
		...
		if self.status not in AMENDMENT_TRANSITIONS:
			frappe.throw(_("Estado de adenda desconocido."))
		if self.amendment_type in {"Increase", "Reduction"} and not (
			money(self.labor_delta) or money(self.material_delta)
		):
			frappe.throw(_("La adenda económica requiere una variación."))
		if not self.is_new() and (previous := self.get_doc_before_save()):
			try:
				assert_transition(str(previous.status), str(self.status), AMENDMENT_TRANSITIONS)
			...
			if previous.status in {"Active", "Superseded", "Cancelled Before Active"}:
				...
				if any(self.get(field) != previous.get(field) for field in immutable):
					frappe.throw(_("La adenda aplicada es inmutable."))

	def on_trash(self) -> None:
		frappe.throw(_("Las adendas no se eliminan."))
