...
class NXRFundAllocation(Document):
    ...
    def validate(self) -> None:
        if money(self.allocated_amount_hnl) <= 0:
            frappe.throw("La asignación debe ser mayor que cero.")
        validate_immutable(
            self,
            (
                "operation", "fund_source", "commitment", "allocated_amount_hnl", "balance_before_hnl",
                "balance_after_hnl", "reserved_before_hnl", "reserved_after_hnl", "allocation_order", "correlation_id",
            ),
        )
