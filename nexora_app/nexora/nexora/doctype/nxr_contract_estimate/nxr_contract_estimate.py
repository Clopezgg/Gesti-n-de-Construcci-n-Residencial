...
from nexora.contracts.core import ESTIMATE_TRANSITIONS, assert_transition, estimate_amounts, money, validate_period
...
class NXRContractEstimate(Document):
    ...
    def validate(self) -> None:
        ...
        values = estimate_amounts(gross, self.advance_amortization, self.retention_amount, self.fine_amount, self.deduction_amount)
        ...
        if previous.status in {"Paid", "Rejected", "Cancelled"}:
            fields = (
                "status", "contract", "estimate_sequence", "period_start", "period_end", "cost_kind", "gross_amount",
                "advance_amortization", "retention_amount", "fine_amount", "deduction_amount", "payable_amount",
                "evidence", "operation", "advance_operation", "paid_at",
            )
