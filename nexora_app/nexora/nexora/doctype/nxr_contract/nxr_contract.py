...
from nexora.contracts.core import CONTRACT_TRANSITIONS, assert_transition, line_amounts, money, validate_period
...
IMMUTABLE_AFTER_ACTIVE = (
    "document_number", "contractor", "contractor_profile", "modality", "project", "cost_center",
    "fund_source", "responsible", "scope", "currency", "exchange_rate", "original_labor_amount",
    "original_material_amount", "original_amount", "start_date", "original_end_date", "idempotency_key",
...
class NXRContract(Document):
    ...
    def validate(self) -> None:
        ...
        if money(self.original_labor_amount) != amounts.labor or money(self.original_material_amount) != amounts.materials:
        ...
        if money(self.current_amount) != money(self.current_labor_amount) + money(self.current_material_amount):
        ...
        if previous.status in {"Active", "Suspended", "Completed", "In Liquidation", *TERMINAL}:
            ...
            if changed:
                frappe.throw(_("El contrato ejecutado es inmutable; use una adenda: {0}").format(", ".join(changed)))
        if previous.status in TERMINAL:
            terminal_fields = (
                "status", "current_scope", "current_labor_amount", "current_material_amount", "current_amount",
                "current_end_date", "version", "executed_labor_amount", "executed_material_amount", "executed_amount", "pending_amount", "paid_amount",
                "advance_disbursed", "advance_amortized", "advance_balance", "retention_held", "retention_returned",
                "retention_balance", "fine_amount", "deduction_amount", "suspension_reason",
            )
