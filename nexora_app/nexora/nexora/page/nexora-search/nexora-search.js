frappe.pages["nexora-search"].on_page_load = function (wrapper) {
	const page = frappe.ui.make_app_page({ parent: wrapper, title: __("Buscador universal"), single_column: true });
	...
	field({
		...
		fieldtype: "Data",
		reqd: 1,
		description: __("Busque por documento de 12 dígitos, nombre, cuenta, referencia, remitente o beneficiario."),
	});
	field({
		fieldname: "scope",
	...
	$(page.body).append(`
		<div class="nxr-search-grid">
			<section class="nxr-card" aria-live="polite"><h3>${__("Resultados")}</h3><div class="nxr-search-results nxr-empty">${__(
				"Escriba un término para comenzar."
			)}</div></section>
			<section class="nxr-card nxr-search-detail" aria-live="polite"><h3>${__(
				"Vista consolidada"
			)}</h3><div class="nxr-search-detail-body nxr-empty">${__("Seleccione un resultado para revisar sus datos, efecto financiero y relaciones.")}</div></section>
		</div>`);
	...
	function renderResults(results) {
		...
		target.removeClass("nxr-empty").append(`<div class="table-responsive"><table class="table table-bordered">
		...
		results.forEach((row) => {
			...
			$(`<tr><td>${frappe.utils.escape_html(ui.term(row.label))}</td><td><a href="${frappe.utils.get_form_link(
				row.doctype,
				row.name
			)}">${frappe.utils.escape_html(row.title)}</a></td><td>${frappe.utils.escape_html(
				row.document_number || __("Sin número")
			)}</td><td>${frappe.utils.escape_html(row.project || __("Sin proyecto"))}</td><td>${frappe.utils.escape_html(
				ui.label("status", row.status)
			)}</td><td>${button}</td></tr>`).appendTo(body);
		});
	}
	...
	$(page.body).on("click", "[data-search-doctype]", async function () {
		...
		try {
			const response = await frappe.call({
				...
				type: "POST",
				args: { payload: { doctype: button.data("search-doctype"), name: button.data("search-name") } },
			});
	...
	function renderDetail(row) {
		const effects = (row.effects || [])
			.map(
				(effect) => `<tr><td>${escape(effect.fund_source)}</td><td>${escape(effect.dimension)}</td><td>${escape(
					effect.effect_type
		...
		const relationships = Object.entries(row.relationships || {})
			.filter(([, value]) => value)
			.map(([key, value]) => `<li><strong>${escape(key.replaceAll("_", " "))}:</strong> ${escape(value)}</li>`)
			.join("");
		$(page.body).find(".nxr-search-detail-body").removeClass("nxr-empty").html(`
			<div class="nxr-preview-summary">
				...
				<span><small>${__("Contraparte")}</small><strong>${escape(row.counterparty || __("Sin contraparte"))}</strong></span>
				<span><small>${__("Importe")}</small><strong>${row.amount_hnl == null ? __("No aplica") : ui.formatMoney(row.amount_hnl, row.currency || "HNL")}</strong></span>
			</div>
			...
			<p><strong>${__("Comprobante")}:</strong> ${row.evidence ? `<a href="${escape(row.evidence)}" target="_blank" rel="noopener">${__("Abrir comprobante")}</a>` : __("Sin comprobante")}</p>
			${relationships ? `<h4>${__("Relaciones e historial")}</h4><ul>${relationships}</ul>` : ""}
			${effects ? `<h4>${__("Efecto financiero")}</h4><div class="table-responsive"><table class="table table-bordered"><thead><tr><th>${__("Fondo")}</th><th>${__("Dimensión")}</th><th>${__("Efecto")}</th><th class="text-right">${__("Importe")}</th></tr></thead><tbody>${effects}</tbody></table></div>` : ""}
			<a class="btn btn-primary" href="${frappe.utils.get_form_link(row.doctype, row.name)}">${__("Abrir documento")}</a>`);
	}
