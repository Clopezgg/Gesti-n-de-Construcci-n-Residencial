frappe.pages["nexora-contracts"].on_page_load = function (wrapper) {
	...
	add({
		fieldname: "profile",
		label: __("Perfil de contratista"),
		fieldtype: "Link",
		options: "NXR Contractor Profile",
	});
	add({
		fieldname: "modality",
		label: __("Modalidad"),
		fieldtype: "Select",
		options: ["Lump Sum", "Unit Price", "Time and Materials", "Labor Only", "Mixed", "Other"],
	});
	add({
	...
	add({
		fieldname: "contract_evidence",
		label: __("Evidencia del contrato"),
		fieldtype: "Link",
		options: "NXR Evidence",
	});
	add({
		fieldname: "signature_evidence",
		label: __("Evidencia de firma"),
		fieldtype: "Link",
		options: "NXR Evidence",
	});
	add({
		fieldname: "approval_evidence",
		label: __("Evidencia de aprobación"),
		fieldtype: "Link",
		options: "NXR Evidence",
	});
	...
	async function publishProject() {
		...
		try {
			...
			console.error("NEXORA contracts failed to publish the active project", error);
		}
	}
		...
		rows.forEach((row) => {
			const button = $(
				`<button class="nxr-ds-btn nxr-ds-btn--secondary nxr-ds-btn--sm nxr-result-row"><strong>${frappe.utils.escape_html(
					...
					row.current_amount
				)} · ${__("Pendiente")}: ${format_currency(row.pending_amount)}</button>`
			);
	...
	function createProfile() {
		dialogAction(
			__("Crear perfil de contratista"),
			[
				{
					fieldname: "entity",
					label: __("Entidad"),
					...
					options: "NXR Entity",
					reqd: 1,
					default: controls.contractor.get_value(),
				},
				{
					fieldname: "classification",
					label: __("Clasificación"),
					fieldtype: "Select",
					options: ["Individual", "Company", "Consortium", "Specialist", "Other"],
					reqd: 1,
					default: "Company",
				},
				{
				...
				{ fieldname: "valid_until", label: __("Vigente hasta"), fieldtype: "Date" },
				{
					...
					fieldtype: "Select",
					options: ["Pending", "Valid", "Expired", "Rejected", "Exception Approved"],
					reqd: 1,
					default: "Pending",
				},
			...
			__("Crear"),
			async (data) => {
				const result = await call("nexora.contracts.service.create_contractor_profile", {
					payload: { ...data, idempotency_key: uuid() },
				});
				controls.profile.set_value(result.profile);
				if (["Valid", "Exception Approved"].includes(data.compliance_status)) {
					await call("nexora.contracts.service.transition_contractor_profile", {
				...
				frappe.show_alert({
					message: __("Perfil {0} creado", [result.document_number]),
					indicator: "green",
				});
			}
		);
	...
	async function createContract() {
		...
		if (labor > 0)
			lines.push({
				line_code: "LAB-001",
				description: __("Mano de obra contractual"),
				cost_kind: "Labor",
				cost_center: controls.cost_center.get_value(),
				fund_source: controls.fund_source.get_value(),
				unit: "Contrato",
				quantity: 1,
				unit_rate: labor,
				amount: labor,
		...
		if (materials > 0)
			lines.push({
				line_code: "MAT-001",
				description: __("Materiales contractuales"),
				cost_kind: "Materials",
				cost_center: controls.cost_center.get_value(),
				fund_source: controls.fund_source.get_value(),
				unit: "Contrato",
		...
		const evidenceRows = [
			...
			.filter((row) => row[1])
			.map((row) => ({ evidence_type: row[0], evidence: row[1] }));
		const result = await call("nexora.contracts.service.create_contract", {
			payload: {
				idempotency_key: uuid(),
				contractor: controls.contractor.get_value(),
				contractor_profile: controls.profile.get_value(),
				modality: controls.modality.get_value(),
				project: controls.project.get_value(),
				cost_center: controls.cost_center.get_value(),
				fund_source: controls.fund_source.get_value(),
				responsible: controls.responsible.get_value(),
				scope: controls.scope.get_value(),
				currency: "HNL",
				exchange_rate: 1,
				start_date: controls.start_date.get_value(),
				end_date: controls.end_date.get_value(),
				signed_on: controls.start_date.get_value(),
				owner_signatory: frappe.session.user_fullname,
				contractor_signatory: controls.contractor.get_value(),
				lines,
		...
		frappe.show_alert({
			message: __("Contrato {0} creado", [result.document_number]),
			indicator: "green",
		});
		await load(selected);
		await refresh();
	...
	function renderActions(doc) {
		...
		actionButton(target, __("Imprimir contrato"), () => {
			window.open(
				`/printview?doctype=NXR%20Contract&name=${encodeURIComponent(
					selected
				)}&format=NEXORA%20Contract&no_letterhead=0`,
				"_blank",
				"noopener"
			);
		});
		...
		if (transitions[doc.status]) {
			actionButton(
				target,
				__("Mover a {0}", [transitions[doc.status]]),
				async () => {
					await call("nexora.contracts.service.transition_contract", {
						contract: selected,
						status: transitions[doc.status],
						idempotency_key: uuid(),
					});
					await load(selected);
					await refresh();
		...
		dialogAction(
			...
			[
				...
				},
				{
					...
					label: __("Fecha efectiva"),
					fieldtype: "Date",
					reqd: 1,
					default: frappe.datetime.get_today(),
				},
				{
					fieldname: "labor_delta",
					label: __("Variación mano de obra"),
					fieldtype: "Currency",
					default: 0,
				},
				{
					fieldname: "material_delta",
					label: __("Variación materiales"),
					fieldtype: "Currency",
					default: 0,
				},
			...
			__("Crear"),
			async (data) => {
				const result = await call("nexora.contracts.service.create_contract_amendment", {
					payload: { ...data, contract: selected, idempotency_key: uuid() },
				});
				frappe.show_alert({
					message: __("Adenda {0} creada", [result.document_number]),
					indicator: "green",
				});
			}
		);
	...
	function applyAmendment() {
		dialogAction(
			__("Aprobar y aplicar adenda"),
			[
				{
					fieldname: "amendment",
					label: __("Adenda"),
					fieldtype: "Link",
					options: "NXR Contract Amendment",
					reqd: 1,
					get_query: () => ({ filters: { contract: selected } }),
				},
			],
			__("Aplicar"),
			async (data) => {
				for (const status of ["In Review", "Approved", "Active"]) {
					await call("nexora.contracts.service.transition_contract_amendment", {
	...
	function createEstimate() {
		dialogAction(
			...
			[
				{
					fieldname: "contract_line",
					label: __("Código de línea contractual"),
					fieldtype: "Data",
					reqd: 1,
				},
				{
					fieldname: "cost_kind",
					label: __("Tipo de costo"),
					fieldtype: "Select",
					options: ["Labor", "Materials"],
					reqd: 1,
				},
				...
				{
					fieldname: "advance_amortization",
					label: __("Amortización"),
					fieldtype: "Currency",
					default: 0,
				},
			...
			__("Crear"),
			async (data) => {
				const result = await call("nexora.contracts.service.create_contract_estimate", {
					payload: {
						...data,
						contract: selected,
						lines: [
							{
								contract_line: data.contract_line,
								description: __("Avance contractual"),
				...
				frappe.show_alert({
					message: __("Estimación {0} creada", [result.document_number]),
					indicator: "green",
				});
			}
		);
	...
	function approveEstimate() {
		dialogAction(
			__("Aprobar estimación"),
			[
				{
					fieldname: "estimate",
					label: __("Estimación"),
					fieldtype: "Link",
					options: "NXR Contract Estimate",
					reqd: 1,
					get_query: () => ({ filters: { contract: selected, status: "Draft" } }),
				},
			],
			__("Aprobar"),
			async (data) => {
				await call("nexora.contracts.service.transition_contract_estimate", {
	...
	function paymentFields(includeEstimate = false) {
		...
		if (includeEstimate)
			fields.push({
				fieldname: "estimate",
				label: __("Estimación aprobada"),
				fieldtype: "Link",
				options: "NXR Contract Estimate",
				reqd: 1,
				get_query: () => ({ filters: { contract: selected, status: "Approved" } }),
			});
		fields.push(
			...
			},
			{
				...
				fieldtype: "Select",
				options: ["Cash", "Transfer", "Cheque", "Other"],
				reqd: 1,
			},
	...
	function disburseAdvance() {
		dialogAction(
			...
			__("Entregar"),
			async (data) => {
				await call("nexora.contracts.service.disburse_contract_advance", {
					payload: {
						...data,
						contract: selected,
						allocations: allocation(data.source, data.amount),
	...
	function payEstimate() {
		dialogAction(
			...
			[
				...paymentFields(true),
				{
					fieldname: "advance_operation",
					label: __("Operación de anticipo para amortizar"),
					fieldtype: "Link",
			...
			__("Pagar"),
			async (data) => {
				await call("nexora.contracts.service.execute_contract_estimate_payment", {
					payload: {
						...data,
						allocations: allocation(data.source, data.amount),
						operation_date: frappe.datetime.get_today(),
	...
	function returnRetention() {
		dialogAction(__("Devolver retención"), paymentFields(), __("Devolver"), async (data) => {
			await call("nexora.contracts.service.return_contract_retention", {
				payload: {
					...data,
					contract: selected,
					allocations: allocation(data.source, data.amount),
	...
	function correctTransaction() {
		dialogAction(
			__("Corregir movimiento contractual"),
			[
				{
					fieldname: "transaction",
					label: __("Movimiento original"),
					fieldtype: "Link",
					options: "NXR Contract Transaction",
					reqd: 1,
					get_query: () => ({ filters: { contract: selected, status: "Executed" } }),
				},
				{
					fieldname: "correction_operation",
					label: __("Corrección"),
					fieldtype: "Select",
					options: ["REAL_RETURN", "REVERSAL_NO_CASH", "DOCUMENT_SUBSTITUTION"],
					reqd: 1,
				},
				{
					fieldname: "amount",
					label: __("Importe de devolución real"),
					fieldtype: "Currency",
					default: 0,
				},
				{
					fieldname: "source",
					label: __("Fuente de devolución"),
					fieldtype: "Link",
					options: "NXR Fund Source",
				},
				{
			...
			__("Corregir"),
			async (data) => {
				await call("nexora.contracts.service.correct_contract_transaction", {
					payload: {
						...data,
						allocations: allocation(data.source, data.amount),
						operation_date: frappe.datetime.get_today(),
	...
	async function initialize() {
		...
		releaseContext = window.nexora.context?.onContextChange?.(async (context) => {
			...
			if ((frappe.get_route ? frappe.get_route() : [])[0] !== "nexora-contracts") return;
