			...
			<div class="table-responsive"><table class="table table-bordered">
				...
				<tbody>${users
					...
					<tr>
						...
						<td>
							<button type="button" class="btn btn-xs btn-default" data-roles="${escape(row.name)}">${__(
							"Roles"
						)}</button>
							<button type="button" class="btn btn-xs btn-default" data-toggle-status="${escape(
								row.name
							)}" data-enabled="${row.enabled ? 1 : 0}">${row.enabled ? __("Desactivar") : __("Activar")}</button>
						</td>
					</tr>`
					)
					.join("")}</tbody>
