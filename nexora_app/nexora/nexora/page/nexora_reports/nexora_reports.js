...
frappe.pages["nexora-reports"].on_page_load = function (wrapper) {
	...
	body.html(`
		<main class="nxr-product-shell nxr-bi-shell" data-state="loading" aria-busy="true">
			...
			<section class="nxr-bi-report-links">
				...
				${reportCard("FI01", __("FI01 · Fondos y remesas"), __("Saldos históricos, transferencias y conciliación"))}
	...
	function renderSummary() {
		...
		body.find(".nxr-control-summary").html(`<div class="nxr-progress-counts"><span><small>${__("Fondos")}</small><strong>${snapshot.analytics?.source_pagination?.total || 0}</strong></span><span><small>${__("Sin conciliar")}</small><strong>${snapshot.analytics?.unreconciled_count || 0}</strong></span><span><small>${__("Contratos")}</small><strong>${snapshot.analytics?.contract_count || 0}</strong></span></div>`);
	}

	function renderTable() {
		const titles = { BI01: __("BI01 · Centro ejecutivo"), FI01: __("FI01 · Fondos y remesas"), FI02: __("FI02 · Consolidado de gastos"), FI03: __("FI03 · Cuentas por pagar"), CO01: __("CO01 · Estado contractual"), PR02: __("PR02 · Presupuesto vs ejecución"), PR03: __("PR03 · Fases y avance"), MM03: __("MM03 · Inventario crítico") };
