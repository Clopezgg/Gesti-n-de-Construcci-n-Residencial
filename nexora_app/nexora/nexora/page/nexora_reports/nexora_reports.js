...
frappe.pages["nexora-reports"].on_page_load = function (wrapper) {
	...
	body.html(`
		<main class="nxr-product-shell nxr-bi-shell" data-state="loading" aria-busy="true">
			...
			<section class="nxr-bi-table-card"><header><div><h3>${__("Reportes guardados")}</h3><small>${__("Filtros persistentes y trazables por usuario")}</small></div><button class="nxr-ds-btn nxr-ds-btn--secondary nxr-ds-btn--sm nxr-closing" data-route="nexora-closing">${__("Cierre semanal")}</button></header><div class="nxr-saved-reports"></div></section>
		</main>
	`);
