frappe.pages["nexora-finance"].on_page_load = function (wrapper) {
	...
	const costCenter = page.add_field({
	...
	const requester = page.add_field({
	...
	const approvedBy = page.add_field({
		label: __("Aprobador"),
	...
	const commitment = page.add_field({
		label: __("Compromiso"),
  ...
  `);
	...
	function operationPayload() {
		return {
			operation_type: operationType.get_value(),
			...
			description: __("Compromiso registrado desde Núcleo de Fondos"),
	...
	async function previewOperation() {
		const response = await frappe.call({
			method: "nexora.financial.service.preview_financial_operation",
			type: "POST",
			args: { payload: operationPayload() },
			freeze: true,
			freeze_message: __("Recalculando saldos en servidor…"),
		});
	...
	function renderPreview(preview) {
      ...
      <table class="table table-bordered"><thead><tr><th>${__("Fuente")}</th><th>${__(
			...
			"Reservado antes"
		)}</th><th>${__("Reservado después")}</th></tr></thead><tbody>${rows}</tbody></table>
      <p><strong>${__("Costo")}:</strong> L${preview.cost_effect_hnl} · <strong>${__(
			"Presupuesto"
		)}:</strong> L${preview.budget_effect_hnl}</p>
