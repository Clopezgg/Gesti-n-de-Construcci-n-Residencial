...
from nexora.financial.db import (
    audit, complete_idempotency, correlation, issue_document_number, link_sequence, operation_doc,
    parse_payload, rollback, savepoint, source_states, start_idempotency,
)
...
def list_source_balances(project: str) -> list[dict[str, str]]:
    ...
    names = frappe.get_all("NXR Fund Source", filters={"project": project, "status": ["in", ["Active", "Exhausted"]]}, pluck="name", order_by="name asc")
    states = source_states(names)
    return [{"source": name, "balance_hnl": f"{states[name].funds:.2f}", "reserved_hnl": f"{states[name].reserved:.2f}", "available_hnl": f"{states[name].available:.2f}"} for name in names]
...
def create_fund_source(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        preview_data = {"preview_hash": canonical_payload_hash({"source": source.name, "amount_hnl": str(source.amount_hnl)}), "sources": []}
        operation = operation_doc(operation_payload, operation_number, fingerprint, preview_data, correlation_id)
        link_sequence(operation_sequence, operation.name)
        with service_write():
            frappe.get_doc({"doctype": "NXR Operation Effect", "operation": operation.name, "fund_source": source.name, "dimension": "Funds", "effect_type": "Received", "amount_hnl": source.amount_hnl, "project": source.project, "correlation_id": correlation_id}).insert(ignore_permissions=True)
        result = {"fund_source": source.name, "source_number": source_number, "operation": operation.name, "document_number": operation_number, "amount_hnl": f"{money(source.amount_hnl):.2f}"}
