...
def create_commitment(payload: str | Mapping[str, Any]) -> dict[str, Any]:
	...
	data.update(
		{"operation_type": "Commitment Reserve", "idempotency_key": str(data.get("idempotency_key") or "")}
	)
	...
	try:
			...
			commitment = frappe.get_doc(
				{
					...
					"document_number": commitment_number,
					"status": "Approved",
					"project": data["project"],
					"commitment_date": data.get("operation_date") or frappe.utils.today(),
					"expiry_date": data.get("expiry_date"),
					"amount_hnl": data["amount_hnl"],
					"beneficiary_doctype": data.get("beneficiary_doctype"),
		...
		operation = operation_doc(
			data, operation_number, fingerprint, preview_data, correlation_id, commitment.name
		)
