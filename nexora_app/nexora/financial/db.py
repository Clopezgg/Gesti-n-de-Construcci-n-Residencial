...
def persist_effects(operation: Any, preview_data: Mapping[str, Any], correlation_id: str, commitment: str | None = None) -> None:
    effect_type = {
        "Outflow": "Executed", "Commitment Reserve": "Reserved", "Commitment Execution": "Executed",
        "Commitment Release": "Released", "Real Return": "Real Return",
    }.get(operation.operation_type, "Reclassification")
    with service_write():
        for order, row in enumerate(preview_data["sources"], start=1):
            frappe.get_doc(
                {
                    "doctype": "NXR Fund Allocation", "operation": operation.name,
                    "fund_source": row["source"], "commitment": commitment,
                    "allocated_amount_hnl": row["amount_hnl"],
                    "balance_before_hnl": row["balance_before_hnl"], "balance_after_hnl": row["balance_after_hnl"],
                    "reserved_before_hnl": row["reserved_before_hnl"], "reserved_after_hnl": row["reserved_after_hnl"],
            ...
            for dimension, delta in (("Funds", Decimal(row["funds_delta_hnl"])), ("Reserved", Decimal(row["reserved_delta_hnl"]))):
                if delta:
                    frappe.get_doc(
                        {
                            "doctype": "NXR Operation Effect", "operation": operation.name,
                            "fund_source": row["source"], "commitment": commitment,
                            "dimension": dimension, "effect_type": effect_type, "amount_hnl": delta,
                            "project": operation.project, "cost_center": operation.cost_center,
                            "economic_category": operation.economic_category, "correlation_id": correlation_id,
                        }
                    ).insert(ignore_permissions=True)
        ...
        for dimension, field, effect in (
            ...
            ("Budget", "budget_effect_hnl", "Budget Reserved" if operation.operation_type == "Commitment Reserve" else "Budget Executed"),
        ):
            delta = Decimal(preview_data[field])
            if delta:
                frappe.get_doc(
                    {
                        "doctype": "NXR Operation Effect", "operation": operation.name, "commitment": commitment,
                        "dimension": dimension, "effect_type": effect, "amount_hnl": delta,
                        "project": operation.project, "cost_center": operation.cost_center,
                        "economic_category": operation.economic_category, "correlation_id": correlation_id,
                    }
                ).insert(ignore_permissions=True)
