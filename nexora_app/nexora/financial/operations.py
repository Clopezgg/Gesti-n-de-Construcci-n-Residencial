...
from nexora.financial.db import (
    ...
    parse_payload, persist_effects, preview, rollback, savepoint, start_idempotency,
)
...
def execute(data: dict[str, Any], *, action: str, commitment: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        result = {"operation": operation.name, "document_number": number, "preview_hash": preview_data["preview_hash"], "sources": preview_data["sources"]}
        audit("financial_operation_executed", "NXR Operation", operation.name, fingerprint, correlation_id, result)
...
def execute_financial_operation(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    action = "return" if operation_type == "Real Return" else "reclassify" if operation_type == "Reclassification" else "execute"
