...
def create_contractor_profile(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        for existing in existing_profiles:
            try:
                overlap = periods_overlap(valid_from, data.get("valid_until"), existing.valid_from, existing.valid_until)
            except ValueError as exc:
                frappe.throw(_(str(exc)))
            if overlap:
        ...
        audit("contractor_profile_created", "NXR Contractor Profile", doc.name, fingerprint, correlation_id, result)
...
def transition_contractor_profile(profile: str, status: str, idempotency_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        if target == "Active":
            ...
            if frappe.utils.getdate(doc.valid_from) > today or (doc.valid_until and frappe.utils.getdate(doc.valid_until) < today):
        ...
        audit("contractor_profile_transitioned", "NXR Contractor Profile", doc.name, fingerprint, correlation_id, result)
        complete_idempotency(idem, "NXR Contractor Profile", doc.name, result)
        return result
    except Exception:
        rollback(point)
...
def create_contract(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    for line in lines:
        ...
        if line.get("fund_source") and frappe.db.get_value("NXR Fund Source", line["fund_source"], "project") != project:
    ...
    try:
        ...
        link_sequence(sequence, doc.name)
        result = {"contract": doc.name, "document_number": number, "status": doc.status, "amounts": _contract_snapshot(doc)}
...
def transition_contract(
    ...
    try:
        ...
        if target in {"Suspended", "Early Terminated"}:
            frappe.throw(_("La suspensión o terminación anticipada debe ejecutarse mediante una adenda versionada."))
        if target == "Cancelled Before Active" and not str(reason or "").strip():
            frappe.throw(_("La transición contractual requiere motivo."))
        if target == "In Liquidation" and money(doc.current_amount) != money(doc.executed_labor_amount) + money(doc.executed_material_amount):
        ...
        result = {"contract": doc.name, "document_number": doc.document_number, "status": doc.status, "amounts": _contract_snapshot(doc)}
...
def create_contract_amendment(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        audit("contract_amendment_created", "NXR Contract Amendment", doc.name, fingerprint, correlation_id, result)
...
def transition_contract_amendment(amendment: str, status: str, idempotency_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        with service_write():
            ...
            doc.save(ignore_permissions=True)
        result = {"amendment": doc.name, "document_number": doc.document_number, "status": doc.status, "contract": _contract_snapshot(contract)}
        audit("contract_amendment_transitioned", "NXR Contract Amendment", doc.name, fingerprint, correlation_id, result)
        complete_idempotency(idem, "NXR Contract Amendment", doc.name, result)
        return result
    except Exception:
        rollback(point)
...
def create_contract_estimate(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        result = {"estimate": doc.name, "document_number": number, "status": doc.status, "gross_amount": str(values.gross), "payable_amount": str(values.payable)}
        audit("contract_estimate_created", "NXR Contract Estimate", doc.name, fingerprint, correlation_id, result)
...
def transition_contract_estimate(estimate: str, status: str, idempotency_key: str) -> dict[str, Any]:
    ...
    try:
        ...
        audit("contract_estimate_transitioned", "NXR Contract Estimate", doc.name, fingerprint, correlation_id, result)
        complete_idempotency(idem, "NXR Contract Estimate", doc.name, result)
        return result
    except Exception:
        rollback(point)
...
def disburse_contract_advance(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    ensure_available(amount, money(contract.current_amount) - money(contract.advance_disbursed), "de anticipo")
    evidence = _validated_evidence(data.get("evidence"), contract.project)
    fingerprint = canonical_payload_hash(data)
    correlation_id = correlation(data)
    point = savepoint()
    try:
        ...
        if cached is not None:
            return cached
        operation = execute_central_operation(_operation_payload(
            contract,
            data,
            key=f"{key}:operation",
        ...
        with service_write():
            ...
            contract.save(ignore_permissions=True)
        movement = _transaction(contract, "Advance", amount, f"{key}:transaction", operation=str(operation["operation"]), evidence=evidence)
        result = {"contract": contract.name, "operation": operation["operation"], "transaction": movement.name, "advance_balance": str(money(contract.advance_balance))}
        audit("contract_advance_disbursed", "NXR Contract", contract.name, fingerprint, correlation_id, result)
...
def execute_contract_estimate_payment(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        if values.payable > 0:
            payment = execute_central_operation(_operation_payload(
                contract,
                data,
                key=f"{key}:payment-operation",
                operation_code="CONSTRUCTION_PAYMENT",
                economic_category=("CONSTRUCTION_LABOR" if estimate.cost_kind == "Labor" else "CONSTRUCTION_MATERIALS"),
                amount=values.payable,
                evidence=evidence,
            ))
            payment_operation = str(payment["operation"])
            _transaction(contract, "Payment", values.payable, f"{key}:payment-transaction", estimate=estimate.name, operation=payment_operation, evidence=evidence)
        settlement_operation = None
        if values.advance_amortization > 0:
            advance_operation = _required(data, "advance_operation", "La amortización requiere la operación de anticipo.")
            settlement = execute_central_operation(_operation_payload(
                contract,
                data,
            ...
            _transaction(contract, "Advance Amortization", values.advance_amortization, f"{key}:settlement-transaction", estimate=estimate.name, operation=settlement_operation, evidence=evidence)
        for transaction_type, amount in (
            ...
            if amount > 0:
                _transaction(contract, transaction_type, amount, f"{key}:{transaction_type.lower()}-transaction", estimate=estimate.name, evidence=evidence)
        with service_write():
            if estimate.cost_kind == "Labor":
        ...
        audit("contract_estimate_paid", "NXR Contract Estimate", estimate.name, fingerprint, correlation_id, result)
...
def return_contract_retention(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        with service_write():
            ...
            contract.save(ignore_permissions=True)
        movement = _transaction(contract, "Retention Return", amount, f"{key}:transaction", operation=str(operation["operation"]), evidence=evidence)
        result = {"contract": contract.name, "operation": operation["operation"], "transaction": movement.name, "retention_balance": str(money(contract.retention_balance))}
        audit("contract_retention_returned", "NXR Contract", contract.name, fingerprint, correlation_id, result)
        complete_idempotency(idem, "NXR Contract", contract.name, result)
        return result
    except Exception:
        rollback(point)
...
def correct_contract_transaction(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
            ...
            if correction_code == "REAL_RETURN":
                if original.transaction_type == "Payment":
                    ...
                    if money(contract.advance_disbursed) < money(contract.advance_amortized):
                        frappe.throw(_("La devolución no puede dejar el anticipo por debajo de lo amortizado."))
                    contract.advance_balance = money(contract.advance_disbursed) - money(contract.advance_amortized)
                ...
                if min(money(contract.paid_amount), money(contract.advance_disbursed), money(contract.retention_returned)) < 0:
        ...
        audit("contract_transaction_corrected", "NXR Contract Transaction", original.name, fingerprint, correlation_id, result)
...
def get_contract(contract: str) -> dict[str, Any]:
    ...
    result.update({
        ...
        "amendments": frappe.get_all("NXR Contract Amendment", filters={"contract": doc.name}, fields=["name", "document_number", "version", "status", "amendment_type", "effective_date", "labor_delta", "material_delta", "new_end_date"], order_by="version desc"),
        "estimates": frappe.get_all("NXR Contract Estimate", filters={"contract": doc.name}, fields=["name", "document_number", "estimate_sequence", "status", "cost_kind", "gross_amount", "payable_amount", "operation"], order_by="estimate_sequence desc"),
        "transactions": frappe.get_all("NXR Contract Transaction", filters={"contract": doc.name}, fields=["name", "document_number", "transaction_type", "transaction_date", "amount", "operation", "estimate", "reference_transaction", "correction_operation"], order_by="creation desc"),
    })
    return result


@frappe.whitelist(methods=["POST"])
def list_contracts(project: str | None = None, contractor: str | None = None, status: str | None = None, limit: int = 50) -> list[dict[str, Any]]:
    require_action("read_contracts")
    ...
    rows = frappe.get_all(
        ...
        fields=["name", "document_number", "status", "contractor", "modality", "project", "current_amount", "executed_amount", "pending_amount", "paid_amount", "advance_balance", "retention_balance", "current_end_date", "version"],
