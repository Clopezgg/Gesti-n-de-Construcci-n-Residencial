...
class TestFinancialCore(unittest.TestCase):
	def test_01_create_hnl_remittance(self) -> None:
		result = validate_source_payload(
			{"channel": "Remittance", "original_amount": "1000", "exchange_rate": 1}
		)
		self.assertEqual("1000.00", result["amount_hnl"])

	def test_02_foreign_currency_conversion(self) -> None:
		result = validate_source_payload(
			{"channel": "Remittance", "original_amount": "100", "exchange_rate": "24.500000000"}
		)
	...
	def test_03_cash_does_not_require_bank(self) -> None:
		...
		with self.assertRaises(FinancialError):
			validate_source_payload(
				{"channel": "Cash", "original_amount": "500", "exchange_rate": 1, "institution": "Banco"}
			)
			...
			preview_operation(
				{
					"operation_type": "Outflow",
					"amount_hnl": 10000,
					"allocations": [{"source": "R-A", "amount_hnl": 6000}],
				},
			...
			preview_operation(
				{
					"operation_type": "Outflow",
					"amount_hnl": 1001,
					"allocations": [{"source": "R-A", "amount_hnl": 1001}],
				},
				...
				ledger.execute(
					{
						"operation_type": "Outflow",
						"amount_hnl": 700,
						"allocations": [{"source": "R-A", "amount_hnl": 700}],
					},
					key,
				)
				return "ok"
	...
	def test_09_same_key_same_payload_returns_same_result(self) -> None:
		...
		payload = {
			"operation_type": "Outflow",
			"amount_hnl": 100,
			"allocations": [{"source": "R-A", "amount_hnl": 100}],
		}
			...
			ledger.execute(
				{
					"operation_type": "Outflow",
					"amount_hnl": 101,
					"allocations": [{"source": "R-A", "amount_hnl": 101}],
				},
				"same",
			)
		...
		result = ledger.execute(
			{
				"operation_type": "Outflow",
				"amount_hnl": 1,
				"allocations": [{"source": "R-A", "amount_hnl": 1}],
			},
			"after-gap",
		)
		...
		ledger.execute(
			{
				...
				"amount_hnl": 400,
				"allocations": [{"source": "R-A", "amount_hnl": 400}],
			},
			"reserve",
		)
	...
	def test_13_commitment_execution_does_not_double_consume_available(self) -> None:
		...
		ledger.execute(
			{
				"operation_type": "Commitment Execution",
				"amount_hnl": 400,
				"allocations": [{"source": "R-A", "amount_hnl": 400}],
			},
			"execute",
	...
	def test_14_release_commitment_restores_available(self) -> None:
		...
		ledger.execute(
			{
				"operation_type": "Commitment Release",
				"amount_hnl": 400,
				"allocations": [{"source": "R-A", "amount_hnl": 400}],
			},
			"release",
		...
		ledger.execute(
			{
				"operation_type": "Outflow",
				"amount_hnl": 200,
				"allocations": [{"source": "R-A", "amount_hnl": 200}],
			},
			"out",
		)
		ledger.execute(
