...
class TestContractContract(unittest.TestCase):
    ...
    def test_doctypes_are_real_and_reuse_entity_finance_and_evidence(self) -> None:
        names = set()
        for folder in (
            "nxr_contractor_profile", "nxr_contract", "nxr_contract_line", "nxr_contract_evidence",
            "nxr_contract_amendment", "nxr_contract_estimate", "nxr_contract_estimate_line",
    ...
    def test_service_exposes_transactional_contract_lifecycle(self) -> None:
        ...
        required = {
            "create_contractor_profile", "transition_contractor_profile", "create_contract", "transition_contract",
            "create_contract_amendment", "transition_contract_amendment", "create_contract_estimate",
            "transition_contract_estimate", "disburse_contract_advance", "execute_contract_estimate_payment",
            "return_contract_retention", "correct_contract_transaction", "get_contract", "list_contracts",
        }
        self.assertTrue(required.issubset(functions))
        text = service_path.read_text(encoding="utf-8")
        for token in (
            "start_idempotency", "issue_document_number", "audit(", "service_write", ".for_update()",
            "execute_central_operation", "_resolve_chain", "NXR Evidence", "NXR Entity",
            "correct_contract_transaction", "REAL_RETURN", "DOCUMENT_SUBSTITUTION",
        ):
            self.assertIn(token, text)
        self.assertNotIn("NXR Contractor\"", text)

    def test_controllers_enforce_service_only_immutability(self) -> None:
        for folder in ("nxr_contractor_profile", "nxr_contract", "nxr_contract_amendment", "nxr_contract_estimate", "nxr_contract_transaction"):
    ...
    def test_permissions_page_workspace_and_hooks_are_connected(self) -> None:
        ...
        page = json.loads((PACKAGE / "nexora/page/nexora_contracts/nexora_contracts.json").read_text(encoding="utf-8"))
