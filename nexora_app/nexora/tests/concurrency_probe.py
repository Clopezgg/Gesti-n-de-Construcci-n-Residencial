...
def run() -> dict[str, object]:
	...
	source = create_fund_source(
		{
			"idempotency_key": f"conc-source-{marker}",
			"source_name": f"Fuente concurrencia {marker}",
			"channel": "Remittance",
			"project": project,
			"currency": "HNL",
			"original_amount": 1000,
			"exchange_rate": 1,
			"origin_or_sender": "Probe concurrencia",
			"custodian": executor,
		}
	)["fund_source"]
	...
	def worker(suffix: str) -> str:
		...
		try:
			...
			execute_financial_operation(
				{
					"idempotency_key": f"conc-{marker}-{suffix}",
			...
			return (
				"denied_insufficient"
				if "disponible suficiente" in str(exc)
	...
	frappe.set_user(executor)  # nosemgrep
	balance = next(row for row in list_source_balances(project) if row["source"] == source)
	if results != ["denied_insufficient", "executed"] or balance["balance_hnl"] != "300.00":
		raise AssertionError({"results": results, "balance": balance})
