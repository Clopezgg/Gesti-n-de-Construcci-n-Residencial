...
class TestExecutiveReconstructionContract(unittest.TestCase):
	...
	def test_weekly_close_is_canonical_and_immutable(self) -> None:
		model = json.loads((APP_ROOT / "nexora/doctype/nxr_weekly_close/nxr_weekly_close.json").read_text(encoding="utf-8"))
		...
		controller = (APP_ROOT / "nexora/doctype/nxr_weekly_close/nxr_weekly_close.py").read_text(encoding="utf-8")
		service = (APP_ROOT / "close/service.py").read_text(encoding="utf-8")
		self.assertIn("require_service_write", controller)
		for marker in ("start_idempotency", "issue_document_number", "savepoint", "rollback", "audit", "DuplicateEntryError"):
			self.assertIn(marker, service)
