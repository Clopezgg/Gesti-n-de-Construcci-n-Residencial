...
class TestExecutiveImprovementsContract(unittest.TestCase):
	def test_analytics_uses_effect_ledger_and_pagination(self) -> None:
		code = "\n".join(
			...
			for path in (
				"dashboard/query_utils.py",
				"dashboard/source_query.py",
				"dashboard/executive.py",
				"dashboard/inventory_query.py",
			)
		)
		for marker in (
	...
	def test_weekly_close_is_canonical_and_immutable(self) -> None:
		model = json.loads(
			(APP_ROOT / "nexora/doctype/nxr_weekly_close/nxr_weekly_close.json").read_text(encoding="utf-8")
		)
