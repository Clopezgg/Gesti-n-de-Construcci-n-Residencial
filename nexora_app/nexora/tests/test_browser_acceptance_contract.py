...
class TestBrowserAcceptanceContract(unittest.TestCase):
	...
	def test_the_navigation_is_built_once_and_updated_by_state(self) -> None:
		...
		self.assertIn("const justBuilt = !intact(shell);\n\t\tif (justBuilt) {\n\t\t\tshell?.remove();\n\t\t\tshell = build();", shell)
	...
	def test_browser_network_calls_and_process_have_explicit_deadlines(self) -> None:
		...
		self.assertIn("kill-after=30s 75m", workflow)
	...
	def test_guided_review_waits_for_stable_network_and_idempotent_rendering(self) -> None:
		...
		for marker in (
			...
			# El repintado sigue siendo idempotente; lo que cambió es que ya no compara
			# contra un estado adivinado por sondeo, sino contra `state.reviewUsable`,
			# que la consola original fija por evento en el instante en que cambia.
