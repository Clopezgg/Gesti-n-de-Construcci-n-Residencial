frappe.ui.form.on("NXR Purchase Order", {
  refresh(frm) {
    ...
    frm.add_custom_button(__("Registrar pago"), () => {
      const dialog = new frappe.ui.Dialog({
        title: __("Registrar pago de compra"),
        fields: [
          { fieldname: "amount_hnl", label: __("Importe HNL"), fieldtype: "Currency", reqd: 1, default: 0, options: "HNL" },
          { fieldname: "economic_category", label: __("Clasificación económica"), fieldtype: "Data", reqd: 1 },
        ...
        primary_action(values) {
          frappe.call({
            ...
            frm.reload_doc();
            if (response.message?.name) frappe.set_route("Form", "NXR Operation", response.message.name);
          }).catch((error) => {
            window.nexora?.ui?.showError?.(error, { title: __("No fue posible ejecutar el pago"), fallback: __("Revise la recepción, compromiso, fondo y permisos.") });
          });
        },
      });
      dialog.show();
