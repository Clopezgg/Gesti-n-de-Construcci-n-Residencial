...
(() => {
	...
	function markup() {
		return `<section class="nxr-guided-wizard" aria-label="${__("Registro guiado de operación")}">
			<ol class="nxr-guided-progress" aria-label="${__("Etapas")}">
				<li><button type="button" data-guided-go="1" aria-current="step"><span>1</span>${__("Datos principales")}</button></li>
			...
			<section class="nxr-guided-stage" data-guided-stage="1"><header tabindex="-1"><p>${__("ETAPA 1")}</p><h3>${__("Datos principales")}</h3><span>${__("Complete la información esencial.")}</span></header><div class="nxr-guided-fields" data-guided-fields="primary"></div><div class="nxr-guided-stage-actions"><button type="button" class="btn btn-primary" data-guided-next="2">${__("Continuar")}</button></div></section>
			<section class="nxr-guided-stage" data-guided-stage="2" hidden><header tabindex="-1"><p>${__("ETAPA 2")}</p><h3>${__("Datos necesarios")}</h3><span>${__("Solo se muestra lo que aplica a esta operación.")}</span></header><div class="nxr-guided-account" aria-live="polite"></div><div class="nxr-guided-fields" data-guided-fields="conditional"></div><div class="nxr-guided-funds"></div><div class="nxr-guided-stage-actions"><button type="button" class="btn btn-default" data-guided-back="1">${__("Atrás")}</button><button type="button" class="btn btn-primary nxr-guided-preview">${__("Revisar operación")}</button></div></section>
			<section class="nxr-guided-stage" data-guided-stage="3" hidden><header tabindex="-1"><p>${__("ETAPA 3")}</p><h3>${__("Revisión")}</h3><span>${__("Compruebe el efecto financiero antes de registrar.")}</span></header><div class="nxr-guided-review nxr-empty">${__("Genere una revisión válida.")}</div><div class="nxr-guided-stage-actions"><button type="button" class="btn btn-default" data-guided-back="2">${__("Corregir datos")}</button><button type="button" class="btn btn-primary" data-guided-next="4" disabled>${__("Continuar al registro")}</button></div></section>
			<section class="nxr-guided-stage" data-guided-stage="4" hidden><header tabindex="-1"><p>${__("ETAPA 4")}</p><h3>${__("Registro definitivo")}</h3><span>${__("NEXORA esperará la confirmación del servidor.")}</span></header><div class="alert alert-warning">${__("El documento ejecutado conservará auditoría y se corregirá mediante movimientos relacionados.")}</div><div class="nxr-guided-final-status" role="status"></div><div class="nxr-guided-stage-actions"><button type="button" class="btn btn-default" data-guided-back="3">${__("Volver")}</button><button type="button" class="btn btn-primary nxr-guided-execute" disabled>${__("Registrar definitivamente")}</button></div></section>
			<details class="nxr-guided-advanced"><summary>${__("Opciones avanzadas")}</summary><p class="text-muted">${__("Clasificación detallada, responsables y datos documentales complementarios.")}</p><div class="nxr-guided-fields" data-guided-fields="advanced"></div><div class="nxr-guided-line-detail"></div></details>
		</section>`;
	}
	...
	function validatePrimary(root, state) {
		...
		qa(state.wizard, ".nxr-guided-primary-error").forEach((node) => node.classList.remove("nxr-guided-primary-error"));
	...
	function bind(root, state) {
		state.wizard.addEventListener("click", (event) => {
			...
			if (go) {
				...
				if (target === 2 && go.hasAttribute("data-guided-next") && !validatePrimary(root, state)) return;
			...
			if (event.target.closest(".nxr-guided-execute")) {
				...
				if (original && !original.disabled && original.getAttribute("aria-busy") !== "true") original.click();
			}
		});
		...
		root.addEventListener("input", (event) => {
			...
			if (["project", "currency", "channel", "payment_method", "origin_or_sender", "beneficiary"].includes(name)) refreshAccounts(root, state);
			if (["currency", "channel", "payment_method", "original_amount", "exchange_rate"].includes(name)) {
				conditional(root, state);
				convertExpense(root);
			}
		});
	...
	function moveFields(root, state, code) {
		...
		(model.CONDITIONAL_FIELDS[code] || []).forEach((name) => move(state, field(root, name), targets.conditional));
		...
		qa(root, ".nxr-document-tabs,.nxr-document-panel,.nxr-detail-tabs,.nxr-detail-panel,.nxr-operational-actions,.nxr-operational-preview").forEach((node) => node.classList.add("nxr-guided-legacy-hidden"));
	}

	function accountMarkup(state) {
		const has = state.accounts.length > 0;
		if (!has) state.choice = "other";
		const options = state.accounts.map((row) => `<option value="${frappe.utils.escape_html(row.name)}"${row.name === state.selected ? " selected" : ""}>${frappe.utils.escape_html(row.label || row.account_name || row.name)}</option>`).join("");
		return `<section class="nxr-human-account-selector"><header><strong>${__("Cuenta para esta operación")}</strong><span>${has ? __("Seleccione una cuenta compatible o use otros datos bancarios.") : __("No hay cuentas compatibles. Complete los datos bancarios.")}</span></header>
			${has ? `<div class="nxr-human-choice" role="radiogroup"><label><input type="radio" name="nxr-guided-account-choice" value="saved"${state.choice === "saved" ? " checked" : ""}> <span>${__("Seleccionar una cuenta guardada")}</span></label><label><input type="radio" name="nxr-guided-account-choice" value="other"${state.choice !== "saved" ? " checked" : ""}> <span>${__("Usar otros datos bancarios")}</span></label></div><label class="nxr-guided-saved-row"${state.choice === "saved" ? "" : " hidden"}><span>${__("Cuenta guardada")}</span><select class="form-control" data-guided-saved-account><option value="">${__("Seleccione una cuenta")}</option>${options}</select></label>` : ""}
			<div class="nxr-save-account-question"${state.choice === "saved" ? " hidden" : ""}><strong>${__("¿Desea guardar esta cuenta para utilizarla nuevamente?")}</strong><div class="nxr-human-choice" role="radiogroup"><label><input type="radio" name="nxr-guided-save-account" value="yes"${state.save ? " checked" : ""}> <span>${__("Sí, guardar para el futuro")}</span></label><label><input type="radio" name="nxr-guided-save-account" value="no"${state.save ? "" : " checked"}> <span>${__("No, usar solo esta vez")}</span></label></div></div><p class="nxr-guided-account-status" role="status"></p></section>`;
	}
	...
	function applyMode(root, state) {
		const mode = model.deriveInternalMode({ choice: state.choice, saveForFuture: state.save, hasAccounts: state.accounts.length > 0 });
		write(root, "account_mode", mode);
		if (mode !== "Existing") write(root, "financial_account", "");
		const locked = mode === "Existing";
		["institution", "account_reference", "currency", model.channelField(movement(root))].forEach((name) => readonly(root, name, locked));
	}

	function switchChoice(root, state, choice) {
		if (state.choice !== "saved") state.draft = { institution: read(root, "institution"), account_reference: read(root, "account_reference"), account_name: read(root, "account_name") };
	...
	async function selectAccount(root, state, name) {
		...
		try {
			const response = await frappe.call({ method: "nexora.financial.service.get_financial_account", type: "POST", args: { account: name, project: ctx.project, direction: model.movementDirection(ctx.movementCode), currency: ctx.currency || null, channel: ctx.channel || null, counterparty: ctx.counterparty || null } });
			...
			applyMode(root, state);
			q(state.wizard, ".nxr-guided-account-status").textContent = __("Cuenta compatible aplicada: {0}", [row.masked_account_reference || row.account_name || name]);
			conditional(root, state);
		} catch (error) {
			state.choice = "other";
			state.selected = "";
			...
			applyMode(root, state);
			renderAccounts(state);
			window.nexora.ui?.showError?.(error, { title: __("No puede utilizarse esa cuenta"), fallback: __("Use una cuenta compatible o complete otros datos bancarios.") });
		}
	}
	...
	async function loadAccounts(root, state, ctx, key, force) {
		...
		try {
			const response = await frappe.call({ method: "nexora.financial.service.list_financial_accounts", type: "POST", args: { project: ctx.project, direction: model.movementDirection(ctx.movementCode), currency: ctx.currency || null, channel: ctx.channel || null, counterparty: ctx.counterparty || null } });
			...
			renderAccounts(state);
			applyMode(root, state);
			q(state.wizard, ".nxr-guided-account-status").textContent = __("No fue posible consultar cuentas autorizadas.");
		}
	}

	function conditional(root, state) {
		const code = movement(root);
		const visible = model.conditionalVisibility({ movementCode: code, channel: read(root, model.channelField(code)), currency: read(root, "currency") || "HNL", accountMode: read(root, "account_mode") || "Manual" });
		Object.entries({ institution: visible.showInstitution, account_reference: visible.showAccountReference, external_reference: visible.showExternalReference, exchange_rate: visible.showExchangeRate, original_amount: visible.showOriginalAmount, account_name: state.choice !== "saved" && visible.showAccountName, evidence: visible.showEvidence, economic_category: visible.showClassification, cost_center: visible.showClassification, payment_method: code === "102" }).forEach(([name, value]) => showField(root, name, Boolean(value)));
	...
	function sync(root, state) {
		...
		const valid = Boolean(preview && originalExecute && !originalExecute.disabled && !preview.classList.contains("nxr-empty"));
		const review = q(state.wizard, ".nxr-guided-review");
		if (review && preview) {
			review.classList.toggle("nxr-empty", !valid);
			review.innerHTML = valid ? preview.innerHTML : frappe.utils.escape_html(preview.textContent || __("Genere una revisión válida."));
		}
		...
		q(state.wizard, ".nxr-guided-final-status").textContent = q(root, ".nxr-action-status")?.textContent || "";
	...
	function install() {
		...
		observer.observe(document.documentElement, { childList: true, subtree: true, characterData: true, attributes: true, attributeFilter: ["disabled", "aria-busy", "class"] });
