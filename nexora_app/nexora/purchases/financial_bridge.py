...
from nexora.financial.commitments import execute_commitment, create_commitment, release_commitment
...
def sync_purchase_order_financials(doc: Any, method: str | None = None) -> None:
    ...
    The transition service remains the authority for workflow permissions. This hook
    only materializes the financial consequence after the status transition has been
    accepted, while staying in the same MariaDB transaction so a failed reservation
    or release rolls the PO change back.
    """
...
def pay_purchase_order(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    """Execute an actual purchase payment through the canonical financial engine.

    Payment is allowed only against an approved/sent/completed PO, a live commitment,
    a real fund source and completed receipt value. The amount is bounded by both the
    received amount and the outstanding commitment; execute_commitment() creates the
    canonical NXR Operation/Effects and applies budget execution atomically.
    """
