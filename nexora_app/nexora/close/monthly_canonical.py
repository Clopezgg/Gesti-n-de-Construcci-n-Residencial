...
from nexora.financial.db import audit, complete_idempotency, correlation, issue_document_number, link_sequence, parse_payload, rollback, savepoint, start_idempotency
...
def _calculate(data: Mapping[str, Any]) -> dict[str, Any]:
    ...
    payload = {
        ...
        "period": {"close_month": close_month, "start_date": start_date.isoformat(), "end_date": end_date.isoformat()},
        "project": project,
        "totals": totals,
...
def create_monthly_close(payload: str | Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        result = {"monthly_close": close.name, "document_number": number, "snapshot_hash": calculated["snapshot_hash"], "engine_version": MONTHLY_ENGINE_VERSION}
...
def list_monthly_closes(payload: str | Mapping[str, Any] | None = None) -> dict[str, Any]:
    ...
    rows = frappe.get_all(
        ...
        fields=["name", "document_number", "status", "project", "close_month", "close_date", "net_flow_hnl", "total_available_hnl", "snapshot_hash", "correction_of"],
