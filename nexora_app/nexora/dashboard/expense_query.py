...
def expense_breakdowns(data: Mapping[str, Any]) -> dict[str, list[dict[str, Any]]]:
	...
	categories = frappe.db.sql(_EXPENSE_CATEGORY_SQL, params, as_dict=True)
	providers = frappe.db.sql(_EXPENSE_PROVIDER_SQL, params, as_dict=True)
	return {
...
def expense_page(data: Mapping[str, Any]) -> dict[str, Any]:
	...
	rows = frappe.db.sql(_EXPENSE_PAGE_SQL, params, as_dict=True)
	totals = frappe.db.sql(_EXPENSE_TOTALS_SQL, params, as_dict=True)[0]
	start, end = _period(data)
	return {
