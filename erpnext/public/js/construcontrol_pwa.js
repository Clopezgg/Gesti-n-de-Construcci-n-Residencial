(() => {
  ...
  function ensureManifestVersion() {
    const link = document.querySelector('link[rel="manifest"][data-cc]') || document.querySelector('link[rel="manifest"]');
    if (link && !link.href.includes(`v=${VERSION}`)) {
      link.href = `/assets/erpnext/construcontrol/manifest.webmanifest?v=${encodeURIComponent(VERSION)}`;
    }
  }
  ...
  function setOfflineBanner(offline) {
    ...
    banner.textContent = "Sin conexión. Los datos no se guardarán hasta recuperar internet.";
  ...
  function offerUpdate(worker) {
    ...
    if (window.frappe?.confirm) {
      frappe.confirm("Hay una versión nueva de ConstruControl. ¿Actualizar ahora?", activate);
    } else {
      activate();
    }
  }
  ...
  async function checkDeployVersion() {
    ...
    try {
      ...
      if (payload.version && payload.version !== VERSION) await registration.update();
  ...
  async function registerServiceWorker() {
    if (!isAppPath() || !isConstruControlRoute() || !('serviceWorker' in navigator) || !window.isSecureContext) return;
    try {
      ...
      if (isConstruControlRoute()) console.warn("ConstruControl PWA registration failed", error);
    }
  }
  ...
  function installControllerReload() {
    ...
    window.addEventListener("pageshow", () => sessionStorage.removeItem(RELOAD_KEY), {once: true});
  }
  ...
  window.addEventListener("online", () => { setOfflineBanner(false); checkDeployVersion(); });
  window.addEventListener("offline", () => setOfflineBanner(true));
  window.addEventListener("load", () => { enhance(); registerServiceWorker(); });
  ...
  if (document.body && window.MutationObserver) {
    ...
    new MutationObserver(() => {
      ...
      scheduled = true;
      window.setTimeout(() => { scheduled = false; configureEvidenceInputs(); }, 80);
