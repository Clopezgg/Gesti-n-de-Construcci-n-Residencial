(() => {
	...
	function calculate(frm) {
		...
		const gross = frappe.utils.flt(
			frm.doc.gross_amount || frm.doc.original_amount || frm.doc.amount_hnl || 0
		);
		...
		const rate =
			currency === "HNL"
				? 1
				: frappe.utils.flt(frm.doc.treasury_exchange_rate || frm.doc.exchange_rate || 1);
	...
	function fallbackBadge(data, channel) {
		...
		return `<div class="cc-institution-badge" style="--cc-institution-color:${frappe.utils.escape_html(
			color
		)}"><span class="cc-institution-mark">${frappe.utils.escape_html(
			initials
		)}</span><span><strong>${frappe.utils.escape_html(name)}</strong><small>${frappe.utils.escape_html(
			CHANNEL_LABELS[channel] || "Ingreso"
		)}</small></span></div>`;
	}
	...
	function renderInstitution(frm) {
		...
		frappe
			.xcall("erpnext.construcontrol.finance.get_institution_visual", { institution })
			.then((data) => {
				const visual = data || {};
				...
				field.$wrapper.html(
					`<div class="cc-institution-badge" style="--cc-institution-color:${frappe.utils.escape_html(
						color
					)}"><span class="cc-institution-logo"><img alt="${frappe.utils.escape_html(
						name
	...
	function updateChannel(frm) {
		const channel = frm.doc.transaction_channel;
		if (channel === "cash" && !frm.doc.financial_institution)
			frm.set_value("financial_institution", "CASH");
