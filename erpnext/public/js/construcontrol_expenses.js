(() => {
  ...
  function updateRequirements(frm) {
    ...
    frm.set_df_property("approved_amount_hnl", "description", approval === "approved" ? "Monto congelado al aprobar" : "Se completa al aprobar");
  }

  function addActions(frm) {
    if (frm.is_new()) return;
    if (["draft", "pending"].includes(frm.doc.professional_approval_status || "draft")) {
      frm.add_custom_button("Enviar a aprobación", () => {
    ...
    frm.add_custom_button("Ver cuentas por pagar", () => frappe.set_route("List", "CC Payable Control"), "ConstruControl");
  }
  ...
  function addSummary(frm) {
    ...
    field.$wrapper.append(`<div class="cc-expense-summary"><div><span>Estado</span><strong>${frappe.utils.escape_html(status)}</strong></div><div><span>Total</span><strong>${total}</strong></div><div><span>Pagado</span><strong>${paid}</strong></div><div><span>Saldo</span><strong>${balance}</strong></div></div>`);
  }
