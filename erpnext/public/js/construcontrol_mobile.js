(() => {
  ...
  const TITLES = {
    ...
    "CC Labor Contract":"Contratos","CC Construction Phase":"Fases","CC Progress Update":"Avances de obra",
    "CC Material Ledger":"Materiales","CC Inventory Movement":"Movimientos de inventario","CC Procurement Request":"Compras",
    "CC Weekly Closing":"Cierres semanales","CC Audit Log":"Auditoría","CC Financial Institution":"Instituciones financieras",
    "CC Generated Report":"Reportes generados","CC Notification Contact":"Contactos de notificación","CC Notification Rule":"Reglas de notificación",
    "CC Notification Log":"Historial de notificaciones","CC Approval Request":"Aprobaciones","CC Evidence":"Evidencias",
    "CC Change Order":"Órdenes de cambio","CC Daily Site Log":"Bitácora diaria","CC Safety Incident":"Seguridad",
    "CC Equipment Control":"Equipos","CC Tool Loan":"Préstamos de herramientas","CC Project Profile":"Proyecto",
    "CC Integration Registry":"Integraciones","CC Crew Attendance":"Asistencia de cuadrilla","CC Crew Member":"Personal de obra"
  };
  const SINGULAR = {
    "CC Funding Source":"ingreso","CC Expense Control":"gasto","CC Payable Control":"cuenta por pagar",
    "CC Labor Contract":"contrato","CC Construction Phase":"fase","CC Progress Update":"avance",
    "CC Material Ledger":"material","CC Inventory Movement":"movimiento","CC Procurement Request":"solicitud de compra",
    "CC Weekly Closing":"cierre","CC Financial Institution":"institución","CC Approval Request":"aprobación",
    "CC Evidence":"evidencia","CC Change Order":"orden de cambio","CC Daily Site Log":"bitácora",
    "CC Safety Incident":"incidente","CC Equipment Control":"equipo","CC Tool Loan":"préstamo"
  };
  ...
  const roleLabel = () => admin() ? "ADMIN" : roleSet().has("ConstruControl Manager") ? "MANAGER" : roleSet().has("ConstruControl Operator") ? "OPERATOR" : roleSet().has("ConstruControl Auditor") ? "AUDITOR" : "VIEWER";
  ...
  const title = route => items().find(row => same(route, row[3]))?.[1] || TITLES[String(route[1] || route[0] || "")] || "ConstruControl";
  ...
  function updateNativePage(current) {
    ...
    if (!friendly) return;
    if (current[0] === "List") {
      document.querySelectorAll(".page-title .title-text, .page-head .title-text").forEach(node => { node.textContent = friendly; });
  ...
  function update(current) {
    ...
    const role = document.querySelector(".cc-profile-role"); if (role) role.textContent = roleLabel();
