(() => {
	...
	const BUSINESS_EVENT_NAME = "construcontrol:business-event:v1";
	const BUSINESS_EVENT_KEYS = [
		"version",
		"project",
		"domain",
		"document_type",
	...
	function route() {
		try {
			return window.frappe ? frappe.get_route() || [] : [];
		} catch (_error) {
			return [];
		}
	}
	...
	function ensureModalCloseButtons() {
		...
		document.querySelectorAll(".modal.show .modal-content").forEach((content) => {
			...
			button.addEventListener("click", () => {
				...
				const standardClose = modal?.querySelector(
					'[data-dismiss="modal"], .btn-modal-close, .modal-header .close'
				);
	...
	function ensureMissingPageRecovery() {
		...
		const missing =
			bodyText.includes("Lamentablemente, no se puede encontrar") ||
	...
	function installBusinessEventRefresh() {
		if (!window.frappe?.realtime?.on) return;
		frappe.realtime.on(BUSINESS_EVENT_NAME, (payload) => {
			if (
				Number(payload?.version) !== 1 ||
				BUSINESS_EVENT_KEYS.some((key) => !Object.hasOwn(payload || {}, key))
			)
				return;
			window.dispatchEvent(
				new CustomEvent("construcontrol:refresh-canonical", { detail: { ...payload } })
			);
		});
	}
