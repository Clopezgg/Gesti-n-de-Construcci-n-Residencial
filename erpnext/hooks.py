...
doc_events = {key: dict(value) for key, value in doc_events.items()}  # noqa: F405
...
for _event in ("after_insert", "on_update", "on_submit", "on_cancel", "on_trash"):
    ...
    if not _existing:
        ...
        _global_events[_event] = (
            [*_existing, _handler] if _handler not in _existing else list(_existing)
        )
...
if isinstance(app_include_css, str):  # noqa: F405
    app_include_css = [app_include_css]  # noqa: F405
else:
    app_include_css = list(app_include_css or [])  # noqa: F405
for _asset in (
...
if isinstance(app_include_js, str):  # noqa: F405
    app_include_js = [app_include_js]  # noqa: F405
else:
    app_include_js = list(app_include_js or [])  # noqa: F405
for _asset in (
