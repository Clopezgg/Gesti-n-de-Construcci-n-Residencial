...
# ConstruControl is kept in an isolated Frappe module so the ERPNext core model
# remains authoritative and upgrades can identify the local extension clearly.
after_migrate = ["erpnext.construcontrol.install.after_migrate"]

# Optional remote file storage.  The implementation transparently falls back to
# Frappe's filesystem unless SUPABASE_STORAGE_MODE=enabled is configured.
write_file = "erpnext.construcontrol.storage.supabase.write_file"
delete_file_data_content = "erpnext.construcontrol.storage.supabase.delete_file"
write_file_keys = ["file_name", "file_url"]
