...
def generate_report_record(
    ...
    values = {
        ...
        "filters_json": json.dumps(
            {"project": project}, ensure_ascii=False, sort_keys=True
        ),
        "totals_json": json.dumps(
            summary["totals"], ensure_ascii=False, sort_keys=True
        ),
    ...
    allowed_fields = {
        field.fieldname for field in frappe.get_meta("CC Generated Report").fields
    }
...
def export_report_csv(
    ...
    _require(
        _EXPORT_ROLES, "No tiene permiso para exportar reportes de ConstruControl."
    )
    ...
    if not profile:
        frappe.throw(
            _(
                "El proyecto no tiene un perfil ConstruControl para proteger la exportación."
            )
        )
    file_name = (
