...
class BusinessRulesTest(unittest.TestCase):
	...
	def test_paid_and_partial_expenses_reconcile_cash_and_balance(self):
		...
		self.assertEqual(
			RULES.expense_amounts(1_000, "partially_paid", "paid", 350, 650, "approved"),
			(1_000, 350, 650),
		)
	...
	def test_cancelled_and_rejected_expenses_do_not_affect_cost_or_cash(self):
		...
		self.assertEqual(
			RULES.expense_amounts(800, "pending_approval", "pending", 0, 800, "rejected"),
			(0, 0, 0),
		)
	...
	def test_drafts_do_not_become_approved_cost(self):
		...
		self.assertEqual(
			RULES.expense_amounts(800, "pending_approval", "pending", 0, 800, "pending"),
			(0, 0, 0),
		)
