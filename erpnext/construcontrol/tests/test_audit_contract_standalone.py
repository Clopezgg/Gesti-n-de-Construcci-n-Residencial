...
class AuditContractTest(unittest.TestCase):
	...
	def test_identical_update_does_not_create_audit_noise(self) -> None:
		...
		current = FakeDocument(
			{"doctype": "CC Project Profile", "name": "CC00", "status": "active"}, previous=previous
		)
	...
	def test_explicit_ignore_flag_prevents_calculation_noise(self) -> None:
		...
		module = load_audit(inserted)
		module.record_event(
			FakeDocument({"doctype": "CC Project Profile", "name": "CC00"}, ignore=True), "on_update"
		)
		self.assertEqual(inserted, [])
	...
	def test_real_change_creates_sanitized_actor_event(self) -> None:
		inserted: list[dict] = []
		module = load_audit(inserted)
		previous = FakeDocument(
			{"doctype": "CC Expense Control", "name": "EXP-1", "amount_hnl": 100, "api_secret": "hidden"}
		)
		current = FakeDocument(
			{"doctype": "CC Expense Control", "name": "EXP-1", "amount_hnl": 150, "api_secret": "hidden"},
			previous=previous,
		)
		module.record_event(current, "on_update")
