...
def load_access(roles: list[str], permissions: list[str] | None = None):
	...
	fake.db = types.SimpleNamespace(
		exists=lambda doctype, name: doctype == "Project" and name in {"P1", "P2"}
	)
...
class AccessContractTest(unittest.TestCase):
	...
	def test_loader_restores_existing_frappe_module(self) -> None:
		sentinel = types.ModuleType("frappe")
		previous = sys.modules.get("frappe")
		sys.modules["frappe"] = sentinel
		try:
			load_access(["ConstruControl Manager"])
