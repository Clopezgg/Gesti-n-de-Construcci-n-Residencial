...
class CommitTitleValidationTest(unittest.TestCase):
	...
	def test_policy_checkpoint_is_the_last_grandfathered_commit(self) -> None:
		self.assertEqual(
			MODULE.POLICY_ENFORCEMENT_SHA,
			"6678c04878be3e6f87d88a871f068e9555415380",
		)

	def test_rejects_invalid_titles_after_checkpoint(self) -> None:
		with patch.object(
			MODULE, "commit_titles", return_value=["fix(ci): enforce future titles", "Legacy title"]
		):
			self.assertEqual(MODULE.invalid_titles(MODULE.commit_titles("base", "head")), ["Legacy title"])
