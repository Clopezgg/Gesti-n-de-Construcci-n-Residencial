...
def fake_install_runtime(
	...
	*,
	...
	frappe.__dict__["flags"] = flags if flags is not None else Flags()
	frappe.__dict__["conf"] = conf if conf is not None else Flags()
	frappe.is_setup_complete = is_setup_complete or (lambda: True)
	...
	frappe_setup_wizard_module.setup_complete = setup_wizard_complete or (lambda _args: {"status": "ok"})
