...
PAGE = (
    ...
    / "construcontrol"
    / "page"
    / "construcontrol_users"
    / "construcontrol_users.js"
)
...
class UsersContractTest(unittest.TestCase):
    ...
    def test_role_labels_are_resolved_by_backend_rules(self) -> None:
        ...
        self.assertEqual(
            module._visible_role({"System Manager", "ConstruControl Viewer"}), "ADMIN"
        )
    ...
    def test_non_admin_cannot_modify_existing_admin_account(self) -> None:
        ...
        module.frappe.db.exists = (
            lambda doctype, filters=None: doctype == "Has Role"
            and filters.get("role") == "System Manager"
        )
    ...
    def test_official_users_center_uses_native_users_and_backend_authorization(
        self,
    ) -> None:
    ...
    def test_user_page_supports_create_edit_suspend_and_project_assignment(
        self,
    ) -> None:
        for phrase in (
            "Nuevo usuario",
    ...
    def test_limited_roles_require_project_scope(self) -> None:
        ...
        for role in (
            ...
            with (
                ...
                self.assertRaisesRegex(ValueError, "Asigne un proyecto"),
            ):
                module._validate_project_assignment(role, "")
        self.assertEqual(
            module._validate_project_assignment("ConstruControl Manager", ""), ""
        )
    ...
    def test_backend_lifecycle_endpoints_are_post_only_and_audited(self) -> None:
        ...
        self.assertGreaterEqual(
            self.service.count('@frappe.whitelist(methods=["POST"])'), 4
        )
        for action in (
            "CREATE",
            "UPDATE",
        ...
        self.assertIn("record_manual_event", self.service)
        self.assertIn('module="US01"', self.service)
        self.assertIn('record_type="User"', self.service)
        self.assertIn('"actor_role"', self.audit)
    ...
    def test_profile_and_direct_page_access_use_real_roles(self) -> None:
        ...
        users_page = next(
            row for row in self.assets["pages"] if row["name"] == "construcontrol-users"
        )
        self.assertEqual(
            users_page["roles"],
