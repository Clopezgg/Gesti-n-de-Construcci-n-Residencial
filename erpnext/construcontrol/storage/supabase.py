...
def _validated_supabase_url(value: str) -> str:
	...
	if (
		...
		frappe.throw(
			_(
				"SUPABASE_URL debe ser una URL HTTPS directa del proyecto con formato https://PROYECTO.supabase.co."
			),
...
def upload_object(bucket: str, object_key: str, content: bytes, content_type: str | None = None) -> None:
	...
	url, key, configured_bucket = _configuration(bucket)
	endpoint = f"{url}/storage/v1/object/{_encoded_object_path(configured_bucket, object_key)}"
	headers = {
		**_auth_headers(key),
...
def download_object_to_path(
	...
	url, key, configured_bucket = _configuration(bucket)
	endpoint = f"{url}/storage/v1/object/authenticated/{_encoded_object_path(configured_bucket, object_key)}"
...
def delete_object(bucket: str, object_key: str) -> None:
	...
	request = Request(
		f"{url}/storage/v1/object/{quote(configured_bucket, safe='')}",
		data=payload,
		headers={
			**_auth_headers(key),
...
def download(object_key: str) -> None:
	...
	if (
		not _enabled()
		or not object_key
		or not object_key.startswith("frappe/")
