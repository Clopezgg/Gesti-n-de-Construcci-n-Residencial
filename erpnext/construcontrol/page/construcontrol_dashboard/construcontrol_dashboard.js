frappe.pages["construcontrol-dashboard"].on_page_load = function (wrapper) {
  ...
  function renderBars(selector, rows) {
    ...
    body.find(selector).html(visible.length ? visible.map(row => `<div class="cc-bar-row"><span class="cc-bar-label" title="${escape(row.label)}">${escape(row.label)}</span><span class="cc-bar-track"><i style="width:${Math.max(((Number(row.amount_hnl) || 0) / max) * 100, 2)}%"></i></span><span class="cc-bar-value">${money(row.amount_hnl)}</span></div>`).join("") : `<span class="cc-empty-state">Sin datos para mostrar.</span>`);
  }

  function renderList(selector, rows, renderer, empty) {
    const visible = (rows || []).slice(0, 3);
    body.find(selector).html(visible.length ? visible.map(renderer).join("") : `<span class="cc-empty-state">${escape(empty)}</span>`);
  }

  function renderAlerts(alerts) {
    const visible = (alerts || []).slice(0, 4);
    body.find("#cc-alerts").html(visible.map((alert, index) => `<button type="button" class="cc-dashboard-alert ${escape(alert.level)}" data-alert-index="${index}"><span class="cc-alert-dot" aria-hidden="true"></span><span><strong>${escape(alert.title)}</strong><small>${escape(alert.message)}</small></span></button>`).join(""));
  }
  ...
  function render(summary) {
    ...
    body.find("#cc-executive-hero p").text(`${summary.progress?.phase_count || 0} fases · ${summary.counts?.contract_count || 0} contratos · ${summary.counts?.expense_count || 0} gastos`);
    body.find("#cc-schedule-status").attr("class", `cc-schedule-status ${escape(summary.progress?.schedule_status || "")}`).text(summary.progress?.schedule_status_label || "En tiempo");
    ...
    renderList("#cc-payables", summary.overdue_payables || [], row => `<button class="cc-dashboard-row" data-form="CC Payable Control" data-name="${escape(row.name)}"><span><strong>${escape(row.provider_name || row.invoice_number || "Cuenta por pagar")}</strong><small>${escape(row.due_date || "Sin vencimiento")}</small></span><span><strong class="is-critical">${money(row.balance_due_hnl)}</strong><small>${escape(row.payable_status_label || "Pendiente")}</small></span></button>`, "No hay cuentas vencidas.");
    renderList("#cc-low-stock", summary.low_stock || [], row => `<button class="cc-dashboard-row" data-form="CC Material Ledger" data-name="${escape(row.name)}"><span><strong>${escape(row.material_name || "Material")}</strong><small>${escape(row.stock_status === "depleted" ? "Agotado" : "Existencia baja")}</small></span><span><strong>${escape(row.current_qty || 0)} ${escape(row.unit || "")}</strong></span></button>`, "No hay materiales críticos.");
    renderList("#cc-recent-activity", summary.recent_activity || [], row => `<button class="cc-dashboard-row" data-list="CC Audit Log"><span><strong>${escape(row.action_label || "Actividad")}</strong><small>${escape(row.actor_name || row.actor_role || "Sistema")}</small></span><span><strong>${escape(row.record_type_label || "Registro")}</strong><small>${escape(row.posting_date || "")}</small></span></button>`, "No hay actividad reciente.");
  }
  ...
  function loadDashboard() {
    ...
    return Promise.resolve()
      ...
      .catch(error => {
        if (requestId !== dashboardRequest) return;
        body.find("#cc-alerts").html(`<div class="cc-dashboard-alert critical"><span><strong>No se pudo cargar el resumen</strong><small>${escape(error?.message || "Revise los datos y permisos del proyecto.")}</small></span></div>`);
      })
      .finally(() => {
  ...
  function openQuickCreate() {
    const dialog = new frappe.ui.Dialog({
      ...
      fields: [{fieldname: "type", fieldtype: "Select", label: "Tipo", options: "Ingreso\nGasto\nAvance\nMovimiento de inventario", reqd: 1}],
      ...
      primary_action(values) {
        ...
        const routes = {
          ...
          "Movimiento de inventario": ["Form", "CC Inventory Movement", "new-cc-inventory-movement-1"],
        };
