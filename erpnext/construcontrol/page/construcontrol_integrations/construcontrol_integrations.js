frappe.pages["construcontrol-integrations"].on_page_load = function (wrapper) {
  ...
  `;
	...
	function icon(row) {
		if (row.icon_file)
			return `<span class="cc-integration-icon" style="--integration-color:${escape(
				row.brand_color || "#175c4c"
			)}"><img alt="" src="${escape(row.icon_file)}"></span>`;
		return `<span class="cc-integration-icon" style="--integration-color:${escape(
			row.brand_color || "#175c4c"
		)}">${escape(initials(row.integration_name))}</span>`;
	}
	function render(rows) {
		...
		body.find("#cc-integrations-grid").html(
			rows.length
				? rows
						.map(
							(row) => `
      <article class="cc-integration-card ${row.enabled ? "" : "is-disabled"}" data-name="${escape(
								row.name
							)}">
        <div class="cc-integration-head">${icon(row)}<div class="cc-integration-name"><strong>${escape(
								row.integration_name
							)}</strong><small>${escape(row.category)} · ${escape(
								row.provider_type
							)}</small></div><span class="cc-integration-status ${escape(
								row.status
							)}">${escape(row.status)}</span></div>
        <p class="cc-integration-description">${escape(row.description || "Sin descripción")}</p>
        <div class="cc-integration-meta"><div><span>Credencial</span><strong>${
			row.credential_configured ? "Configurada" : "No configurada"
		}</strong></div><div><span>Última prueba</span><strong>${escape(
								row.last_test_status || "not_tested"
							)}</strong></div></div>
        ${
			row.is_protected
				? `<div class="cc-integration-protected">Integración esencial protegida</div>`
				: ``
		}
        <div class="cc-integration-actions">
          <button class="btn btn-sm ${
				row.enabled ? "btn-default" : "btn-primary"
			}" data-action="toggle" data-enabled="${row.enabled ? 0 : 1}">${
          ...
          ${
				row.is_protected
					? ``
					: `<button class="btn btn-sm btn-default" data-action="archive">Archivar</button><button class="btn btn-sm btn-danger" data-action="delete">Eliminar</button>`
			}
        </div>
      </article>`
						)
						.join("")
				: `<div class="text-muted">No hay integraciones registradas.</div>`
		);
	}
	function load(includeArchived = false) {
		return frappe
			.xcall("erpnext.construcontrol.integrations.list_integrations", {
				include_archived: includeArchived ? 1 : 0,
			})
			.then(render);
	}
	function selected(button) {
		return $(button).closest(".cc-integration-card").data("name");
	}
	function createIntegration() {
		frappe.prompt(
			...
			(values) =>
				frappe
					.xcall("erpnext.construcontrol.integrations.create_custom_integration", values)
					.then(() => load()),
			"Nueva integración",
			"Crear"
		);
	...
	body.on("click", "[data-action]", function () {
		...
		if (action === "toggle")
			return frappe
				.xcall("erpnext.construcontrol.integrations.set_integration_enabled", {
					name,
		...
		if (action === "test")
			return frappe
				.xcall("erpnext.construcontrol.integrations.test_integration", { name })
				.then((row) => {
					frappe.msgprint({
						...
						indicator:
							row.last_test_status === "passed"
								? "green"
								: row.last_test_status === "warning"
								? "orange"
								: "red",
						message: escape(row.last_test_message),
					});
					load();
				});
		if (action === "archive")
			return frappe.confirm("¿Archivar esta integración personalizada?", () =>
				frappe
					.xcall("erpnext.construcontrol.integrations.archive_integration", { name, archived: 1 })
					.then(() => load())
			);
		if (action === "delete")
			return frappe.prompt(
				[{ fieldname: "confirmation", fieldtype: "Data", label: "Escriba ELIMINAR", reqd: 1 }],
				(values) =>
					frappe
						.xcall("erpnext.construcontrol.integrations.delete_custom_integration", {
							name,
