frappe.pages["construcontrol-project-center"].on_page_load = function (wrapper) {
  ...
  function load() {
    ...
    return Promise.resolve()
      .then(() => frappe.xcall("erpnext.construcontrol.construction.get_project_center", {project: activeProject}))
      .then(summary => {
      ...
      .catch(error => {
        if (requestId !== projectRequest) return;
        body.find("#cc-project-hero").html(`<div><h2>No se pudieron actualizar los indicadores</h2><p class="text-danger">${escape(error?.message || "Revise los datos y permisos del proyecto.")}</p></div>`);
      })
      .then(() => {
