frappe.pages["construcontrol-project-center"].on_page_load = function (wrapper) {
	...
	page.set_primary_action("Actualizar resumen", () => load(), "refresh");

	body.html(`
    <div class="cc-project-center" aria-busy="false">
      <div id="cc-project-refresh-state" class="cc-project-refresh-state" hidden>Actualizando indicadores...</div>
  ...
  `;
	...
	function render(summary) {
		...
		if (!summary.project) {
			body.find("#cc-project-hero").html(
				`<div><h2>Sin proyecto activo</h2><p class="text-muted">Configure un perfil de proyecto para comenzar.</p></div>`
			);
			body.find(
				"#cc-project-metrics,#cc-phase-list,#cc-contract-list,#cc-material-list,#cc-progress-list"
			).empty();
			return;
		}
		const statusClass =
			summary.alert_level === "critical"
				? "is-critical"
				: summary.alert_level === "attention"
				? "is-attention"
				: "";
		body.find("#cc-project-hero").html(
			`<div><small class="text-muted">PROYECTO ACTIVO</small><h2>${escape(
				summary.project_name || summary.project
			)}</h2><p class="text-muted">${summary.phase_count || 0} fases · ${
				summary.delayed_phase_count || 0
			} atrasadas · ${
				summary.at_risk_phase_count || 0
			} en riesgo</p></div><span class="cc-project-status ${statusClass}">${escape(
				label(summary.schedule_status || "on_track")
			)}</span>`
		);
		...
		body.find("#cc-project-metrics").html(
			metrics
				.map(
					(row) =>
						`<div class="cc-project-metric"><span>${escape(row[0])}</span><strong>${
							row[1]
						}</strong></div>`
				)
				.join("")
		...
		body.find("#cc-phase-list").html(
			phases.length
				? phases
						.map(
							(row) =>
								`<button class="cc-phase-row" data-doctype="CC Construction Phase" data-name="${escape(
									row.name
								)}"><span class="cc-row-label"><strong>${escape(
									row.phase_name
								)}</strong><small>${escape(
									label(row.schedule_status)
								)}</small></span><span><small class="text-muted">Presupuesto</small><strong>${money(
									row.budget_hnl
								)}</strong></span><span><small class="text-muted">Costo real</small><strong>${money(
									row.actual_cost_hnl
								)}</strong></span><span><small class="text-muted">Físico</small><strong>${
									row.physical_progress_percent || 0
								}%</strong></span><span><small class="text-muted">Financiero</small><strong>${
									row.financial_progress_percent || 0
								}%</strong></span><span class="cc-progress-bar"><i style="width:${Math.min(
									Math.max(row.physical_progress_percent || 0, 0),
									100
								)}%"></i></span></button>`
						)
						.join("")
				: `<span class="text-muted">No hay fases registradas.</span>`
		);

		const contracts = summary.contracts || [];
		body.find("#cc-contract-list").html(
			`<div class="cc-simple-list">${
				contracts.length
					? contracts
							.map(
								(row) =>
									`<button class="cc-simple-row" data-doctype="CC Labor Contract" data-name="${escape(
										row.name
									)}"><span><strong>${escape(
										row.contractor_name || row.contract_code
									)}</strong><small>${escape(
										label(row.status || "")
									)}</small></span><span><strong>${money(
										row.balance_hnl
									)}</strong><small>Saldo</small></span></button>`
							)
							.join("")
					: `<span class="text-muted">No hay contratos registrados.</span>`
			}</div>`
		);

		const materials = summary.materials || [];
		body.find("#cc-material-list").html(
			`<div class="cc-simple-list">${
				materials.length
					? materials
							.map(
								(row) =>
									`<button class="cc-simple-row" data-doctype="CC Material Ledger" data-name="${escape(
										row.name
									)}"><span><strong>${escape(
										row.material_name
									)}</strong><small class="cc-stock-${escape(row.stock_status)}">${escape(
										label(row.stock_status || "available")
									)}</small></span><span><strong>${escape(row.current_qty || 0)} ${escape(
										row.unit || ""
									)}</strong><small>Existencia</small></span></button>`
							)
							.join("")
					: `<span class="text-muted">No hay materiales registrados.</span>`
			}</div>`
		);

		const progress = summary.recent_progress || [];
		body.find("#cc-progress-list").html(
			`<div class="cc-simple-list">${
				progress.length
					? progress
							.map(
								(row) =>
									`<button class="cc-simple-row" data-doctype="CC Progress Update" data-name="${escape(
										row.name
									)}"><span><strong>${escape(
										row.title || "Actualización de avance"
									)}</strong><small>${escape(row.posting_date || "")} · ${escape(
										row.responsible || ""
									)}</small></span><span><strong>${escape(
										row.progress_percent || 0
									)}%</strong><small>${escape(row.quality || "")}</small></span></button>`
							)
							.join("")
					: `<span class="text-muted">No hay actualizaciones recientes.</span>`
			}</div>`
		);
	}

	function setLoading(loading) {
		body.find(".cc-project-center").attr("aria-busy", loading ? "true" : "false");
		body.find("#cc-project-refresh-state").prop("hidden", !loading);
	}
	...
	function load() {
		...
		return Promise.resolve()
			.then(() =>
				frappe.xcall("erpnext.construcontrol.construction.get_project_center", {
			...
			.catch((error) => {
				if (requestId !== projectRequest) return;
				body.find("#cc-project-hero").html(
					`<div><h2>No se pudieron actualizar los indicadores</h2><p class="text-danger">${escape(
						error?.message || "Revise los datos y permisos del proyecto."
					)}</p></div>`
				);
			})
			.then(() => {
				if (requestId === projectRequest) setLoading(false);
			});
	}
	...
	window.addEventListener("construcontrol:refresh-canonical", (event) => {
		const changedProject = event.detail?.project || null;
		if (!activeProject || !changedProject || changedProject === activeProject) load();
	});
