frappe.pages["construcontrol-users"].on_page_load = function (wrapper) {
  ...
  `);
	...
	function render() {
		...
		body.find("#cc-users-counts").html(
			`<span><strong>${users.length}</strong> usuarios</span><span><strong>${active}</strong> activos</span><span><strong>${suspended}</strong> suspendidos</span>`
		);

		if (!users.length) {
			body.find("#cc-users-list").html(
				`<div class="cc-empty-state">No se encontraron usuarios con los filtros seleccionados.</div>`
			);
			return;
		}
		...
		body.find("#cc-users-list").html(
			users
				.map((row) => {
					const project =
						(row.projects || []).join(", ") || "Todos los proyectos autorizados por rol";
					const historical = row.historical_source_id
						? `<span class="cc-user-history">Origen histórico conciliado</span>`
						: "";
					const protectedLabel = row.protected
						? `<span class="cc-user-protected">Protegido</span>`
						: "";
					const action = row.protected
						? `<button type="button" class="btn btn-xs btn-default" data-profile="${escape(
								row.user_id
						  )}">Perfil</button>`
						: `<button type="button" class="btn btn-xs btn-default" data-edit-user="${escape(
								row.user_id
						  )}">Editar</button><button type="button" class="btn btn-xs ${
								row.enabled ? "btn-default" : "btn-primary"
						  }" data-toggle-user="${escape(row.user_id)}" data-enabled="${
								row.enabled ? 0 : 1
						  }">${row.enabled ? "Suspender" : "Reactivar"}</button>`;
					return `<article class="cc-user-row">
        <div class="cc-user-avatar">${
			row.user_image
				? `<img src="${escape(row.user_image)}" alt="">`
				: escape(initials(row.display_name))
		}</div>
        <div class="cc-user-main"><div class="cc-user-title"><strong>${escape(
			row.display_name
		)}</strong><span class="cc-user-role">${escape(
						row.role
					)}</span>${protectedLabel}${historical}</div><span>${escape(
						row.email
					)}</span><small>Proyecto: ${escape(project)}</small></div>
        <div class="cc-user-access"><span class="${row.enabled ? "is-active" : "is-disabled"}">${
						row.enabled ? "Activo" : "Suspendido"
					}</span><small>Último acceso: ${escape(
						formatMoment(row.last_login || row.last_active)
					)}</small></div>
	...
	function loadUsers() {
		...
		setLoading(true);
		return frappe
			.xcall("erpnext.construcontrol.users.get_user_center", {
		...
		const dialog = new frappe.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					default: user?.projects?.[0] || "",
				},
				{
					fieldname: "enabled",
					fieldtype: "Check",
					label: "Cuenta activa",
					default: user ? user.enabled : 1,
				},
			],
			...
			primary_action(values) {
				...
				frappe
					.xcall("erpnext.construcontrol.users.save_user", values)
					.then(() => {
						dialog.hide();
						frappe.show_alert({
							message: user ? "Usuario actualizado" : "Usuario creado",
							indicator: "green",
						});
						return loadUsers();
					})
	...
	body.on("click", "[data-edit-user]", function () {
		openUserDialog(String($(this).data("edit-user")));
	});
