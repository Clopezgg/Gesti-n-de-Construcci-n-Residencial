  ...
  `);
	...
	async function loadProjects() {
		try {
			...
			body.find("#cc-cl-project").append(
				rows
					.map(
						(row) =>
							`<option value="${esc(row.name)}">${esc(row.project_name || row.name)}</option>`
					)
					.join("")
			);
		} catch (error) {
	...
	body.find("#cc-cl-preview").on("click", async () => {
		...
		try {
			...
			body.find("#cc-cl-result").html(
				`<div class="cc-cl-metrics">${metrics
					.map(
						(row) =>
							`<div class="cc-cl-metric"><span class="text-muted">${row[0]}</span><strong>${row[1]}</strong></div>`
					)
					.join("")}</div><div class="cc-cl-panel"><h3>Pendientes</h3>${
					(snapshot.pending_items || []).length
						? `<ul>${snapshot.pending_items.map((item) => `<li>${esc(item)}</li>`).join("")}</ul>`
						: `<p class="text-muted">Sin pendientes detectados.</p>`
				}</div>`
			);
			body.find("#cc-cl-create").prop("disabled", false);
		} catch (error) {
			body.find("#cc-cl-result").html(
				`<div class="cc-cl-panel text-danger">${esc(error?.message || error)}</div>`
			);
		}
	});
