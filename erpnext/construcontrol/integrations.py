...
def test_integration(name: str) -> dict[str, Any]:
    ...
    if provider == "erpnext_core":
        passed = bool(frappe.db.exists("DocType", "Project") and frappe.db.exists("DocType", "CC Project Profile"))
        message = "Motor ERPNext y extensión ConstruControl disponibles." if passed else "Faltan componentes centrales del sistema."
    elif provider == "email":
        passed = bool(frappe.db.exists("Email Account", {"enable_outgoing": 1}))
        message = "Existe una cuenta de correo saliente activa." if passed else "No hay una cuenta de correo saliente activa."
    elif provider == "supabase":
        passed = bool(frappe.db.exists("ConstruControl Migration Run", {"dry_run": 0, "status": ["in", ["Completed", "Completed with Warnings"]]}))
        message = "La migración histórica de Supabase está registrada y conciliada." if passed else "No se encontró una migración histórica conciliada."
    else:
