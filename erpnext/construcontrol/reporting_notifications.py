...
def _default_message(
    event_type: str, contact_name: str, summary: dict[str, Any]
) -> str:
...
def prepare_notification(
    ...
    if not document.has_permission("read"):
        frappe.throw(
            _("No tiene permiso para utilizar este contacto."), frappe.PermissionError
        )
    if document.meta.has_field("project") and document.get("project") not in {
        ...
        frappe.throw(
            _("El contacto pertenece a otro proyecto."), frappe.PermissionError
        )
    active = int(document.get("active") if document.meta.has_field("active") else 1)
    authorized = int(
        document.get("authorized") if document.meta.has_field("authorized") else 0
    )
    if not active or not authorized:
        frappe.throw(
            _(
                "El contacto debe estar activo y autorizado antes de preparar una notificación."
            )
        )
    ...
    summary = get_reporting_summary(
        date_from=date_from, date_to=date_to, project=project
    )
    contact_name = str(
        document.get("contact_name") or document.get("title") or "contacto"
    )
    body = str(message or "").strip() or _default_message(
        str(event_type), contact_name, summary
    )
    if len(body) > 1500:
    ...
    allowed_fields = {
        field.fieldname for field in frappe.get_meta("CC Notification Log").fields
    }
    log = frappe.get_doc(
        {
            key: value
            for key, value in values.items()
            if key == "doctype" or key in allowed_fields
        }
    )
    log.insert()
    return {
...
def mark_notification_sent(log_name: str) -> dict[str, Any]:
    ...
    if not log.has_permission("write"):
        frappe.throw(
            _("No tiene permiso para modificar este registro."), frappe.PermissionError
        )
