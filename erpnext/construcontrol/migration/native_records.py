...
def _construction_group(record: Mapping[str, Any]) -> str | None:
	...
	haystack = " ".join(
		str(record.get(field) or "") for field in ("category", "subcategory", "name", "title", "description")
	).casefold()
...
def _uom(record: Mapping[str, Any]) -> str | None:
	...
	whole = int(
		normalized
		in {"unidad", "unidades", "und", "unid", "pieza", "piezas", "saco", "sacos", "bolsa", "bolsas"}
	)
	return (
		frappe.get_doc({"doctype": "UOM", "uom_name": raw, "must_be_whole_number": whole})
		.insert(ignore_permissions=True)
		.name
	)
