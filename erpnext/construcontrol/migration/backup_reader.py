...
def parse_copy_tables(sql_text: str, wanted: set[tuple[str, str]] | None = None) -> dict[tuple[str, str], list[dict[str, Any]]]:
...
def _read_backup_bytes(content: bytes, file_name: str) -> tuple[Any, str]:
	...
	if lower.endswith((".tar.gz", ".tgz")):
		with tarfile.open(fileobj=io.BytesIO(content), mode="r:gz") as archive:
			members = [m for m in archive.getmembers() if m.isfile() and Path(m.name).name == "data.sql"]
			if len(members) != 1:
			...
			return _payload_from_sql(extracted.read().decode("utf-8-sig")), "Supabase Logical Backup"
	if lower.endswith(".gz"):
		return _payload_from_sql(gzip.decompress(content).decode("utf-8-sig")), "Supabase SQL Gzip"
