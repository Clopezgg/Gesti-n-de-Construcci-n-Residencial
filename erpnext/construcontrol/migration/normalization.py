...
def normalize_movement_type(value: Any) -> str:
	...
	if text in outgoing:
		return (
			"consumption" if text in {"consumption", "consume", "use", "issue", "exit"} else "adjustment_out"
		)
	return "consumption"
	...
	for snapshot in snapshots:
		...
		for record in accounts:
			...
			identity = {
				...
				"display_name": str(
					record.get("name") or record.get("displayName") or record.get("email") or ""
				),
				"role": role,
			}
			for key in _directory_keys(record):
				current = directory.get(key)
				if current is None or ROLE_PRIORITY.get(role, 0) > ROLE_PRIORITY.get(
					current.get("role", "USER"), 0
				):
...
def resolve_actor_identity(
	record: Mapping[str, Any], directory: Mapping[str, Mapping[str, str]]
) -> dict[str, str]:
	...
	display_name = str(
		record.get("actorName") or record.get("displayName") or record.get("name") or email or actor
	)
	return {
