...
def _build_summary(rows: dict[str, list[Any]], project: str | None, start: Any, end: Any) -> dict[str, Any]:
	funds = rows["funds"]
	...
	for row in funds:
		...
		if amount:
			channel = str(row.get("transaction_channel") or row.get("income_type") or "other")
			income_channels[channel] = round(income_channels.get(channel, 0.0) + amount, 2)
	...
	for row in expenses:
		...
		if row_recognized:
			...
			categories[category] = round(categories.get(category, 0.0) + row_recognized, 2)
			providers[provider] = round(providers.get(provider, 0.0) + row_recognized, 2)

	active_contracts = [row for row in contracts if _active_contract(row)]
	contracted = round(
		sum(flt(row.get("project_value_hnl") or row.get("labor_value_hnl")) for row in active_contracts),
		2,
	)
	contract_paid = round(sum(flt(row.get("paid_hnl")) for row in active_contracts), 2)
	contract_balance = round(sum(flt(row.get("balance_hnl")) for row in active_contracts), 2)
	...
	inventory_value = round(
		sum(flt(row.get("current_qty")) * flt(row.get("unit_cost_hnl")) for row in materials),
		2,
	)
	low_stock = [row for row in materials if str(row.get("stock_status") or "") in {"low", "depleted"}]
	quality_issues = [
		row
	...
	closed_closings = [row for row in closings if str(row.get("status") or "").casefold() == "closed"]
	...
	return {
		...
		"income_channels": [
			{"label": label, "amount_hnl": amount}
			for label, amount in sorted(income_channels.items(), key=lambda item: item[1], reverse=True)
		],
		"providers": [
			{"label": label, "amount_hnl": amount}
...
def get_reporting_summary(
	...
	summary = _build_summary(_source_rows(start, end, scoped), resolved_project, start, end)
