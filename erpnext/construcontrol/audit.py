...
def _changed_to(
    previous: dict[str, Any], following: dict[str, Any], values: set[str]
) -> bool:
    for fieldname in (
...
def _derive_action(
    method: str | None, previous: dict[str, Any], following: dict[str, Any]
) -> str:
    if method == "after_insert":
        return "CREATE"
    if method == "on_trash":
        return "DELETE"
    if method == "on_cancel" or _changed_to(
        previous, following, {"cancelled", "canceled", "annulled"}
    ):
        return "CANCEL"
    ...
    if _changed_to(previous, following, {"paid", "partially_paid", "partial"}):
        return "PAY"
    if method == "on_submit" or _changed_to(
        previous, following, {"approved", "verified", "closed"}
    ):
        return "APPROVE"
    return "UPDATE"
...
def _insert_event(values: dict[str, Any]) -> None:
    if hasattr(frappe, "get_meta"):
        ...
        payload = {
            ...
            if key == "doctype" or key in allowed
        }
    else:
        payload = dict(values)
    ...
    *,
    ...
    return {
        ...
        "previous_state": json.dumps(
            previous_state, ensure_ascii=False, sort_keys=True, default=str
        ),
        "next_state": json.dumps(
            next_state, ensure_ascii=False, sort_keys=True, default=str
        ),
        "reason": reason,
        "origin": origin,
        "correlation_id": correlation_id,
...
def record_event(doc: Any, method: str | None = None) -> None:
    ...
    if (
        ...
        return
    if method == "on_update" and getattr(
        getattr(doc, "flags", None), "in_insert", False
    ):
        return
    ...
    values = _event_values(
        ...
        if hasattr(doc, "get") and doc.get("project")
        else None,
...
def protect_audit_record(doc: Any, method: str | None = None) -> None:
    ...
    if method == "on_trash" or not getattr(doc, "is_new", lambda: False)():
        frappe.throw(
            _("La auditoría de ConstruControl es inmutable."), frappe.PermissionError
        )
    frappe.throw(
