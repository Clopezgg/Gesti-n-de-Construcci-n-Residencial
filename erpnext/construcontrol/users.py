...
def _assert_admin_transition(
    ...
    *,
    ...
    if user == "Administrator":
        _throw(
            "La cuenta Administrator no puede suspenderse, degradarse ni eliminarse."
        )
    if user == frappe.session.user:
        _throw(
            "No puede retirar privilegios administrativos de la cuenta que está utilizando."
        )
    _require_system_manager()
    if not _enabled_system_managers(exclude=user):
        _throw(
            "La última cuenta ADMIN protegida no puede suspenderse, degradarse ni eliminarse."
        )
...
def _role_from_label(label: str) -> str:
    ...
    if not role:
        frappe.throw(
            _(
                "Seleccione un rol permitido: ADMIN, MANAGER, OPERATOR, AUDITOR o VIEWER."
            )
        )
...
def _user_roles(users: list[str]) -> dict[str, set[str]]:
    ...
    for row in frappe.get_all(
        ...
        filters={
            "parent": ["in", users],
            "role": ["in", ["System Manager", *BUSINESS_ROLES]],
        },
        fields=["parent", "role"],
    ):
...
@frappe.whitelist()
def get_user_center(
    search: str = "", enabled: str | int | None = None
) -> dict[str, Any]:
    _require_management()
...
def _set_business_role(doc: Any, role: str) -> None:
    keep = [
        ...
        if row.role not in {"System Manager", *BUSINESS_ROLES}
    ]
    doc.set("roles", [])
...
def _set_project_permission(user: str, project: str) -> None:
    for name in frappe.get_all(
        "User Permission", filters={"user": user, "allow": "Project"}, pluck="name"
    ):
