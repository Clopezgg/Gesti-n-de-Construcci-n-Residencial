...
def _require_admin() -> None:
	if "System Manager" not in set(frappe.get_roles()):
		frappe.throw(
			_("Solo un administrador puede ejecutar la comprobación integral."), frappe.PermissionError
		)
...
def run(require_migration: bool = False, require_no_demo: bool = False) -> dict[str, Any]:
	...
	for fieldname in (
		"require_backup_before_import",
		"cleanup_demo_after_migration",
	...
	for fund in funds:
		...
		spent = round(
			sum(
				flt(row.amount_hnl)
				for row in active
				if row.status != "pending" and row.financial_status != "pending"
			),
			2,
		)
		pending = round(
		...
		if (
			abs(spent - flt(fund.spent_hnl)) > 0.01
			or abs(pending - flt(fund.pending_hnl)) > 0.01
			or abs(available - flt(fund.available_hnl)) > 0.01
		):
	...
	for contract in contracts:
		active = [
			row
			for row in expenses_by_contract.get(contract.name, [])
			if _active_expense(row) and row.financial_status != "pending"
		]
	...
	if (
		operator_migration.get("create")
		or operator_migration.get("write")
		or operator_migration.get("delete")
	):
