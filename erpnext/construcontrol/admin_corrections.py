...
_ALLOWED_FIELDS = {
    "project", "phase", "title", "posting_date", "category", "subcategory",
    "commercial_source", "unit", "quantity", "description", "provider_name",
    "supplier", "payment_method", "funding_source", "labor_contract",
    "labor_payment_type", "subtotal_hnl", "tax_hnl", "withholding_hnl",
    "discount_hnl", "paid_amount_hnl", "payment_status",
    "professional_approval_status", "invoice_number", "invoice_date", "due_date",
    "purchase_order_reference", "cost_center", "notes",
}
_FINANCIAL_FIELDS = {
    "subtotal_hnl", "tax_hnl", "withholding_hnl", "discount_hnl",
    "paid_amount_hnl", "payment_status", "professional_approval_status",
    "funding_source", "labor_contract",
}
_OPERATIONS = {"correct", "annul_migrated", "reverse_imported_payment", "register_reimbursement"}
_SNAPSHOT_FIELDS = tuple(sorted(_ALLOWED_FIELDS | {
    "name", "source_id", "source_key", "calculated_total_hnl", "amount_hnl",
    "balance_due_hnl", "approved_amount_hnl", "approval_status", "financial_status",
    "status", "is_logically_deleted",
}))


def _require_administrator() -> None:
    if str(frappe.session.user or "") != "Administrator":
        frappe.throw(_("Solo la cuenta Administrator puede ejecutar correcciones críticas."), frappe.PermissionError)
...
def _security_status(doc: Any | None = None) -> dict[str, Any]:
    ...
    locked_until = frappe.utils.get_datetime(_get(doc, "correction_locked_until")) if _get(doc, "correction_locked_until") else None
    return {
...
def _auth_failure(message: str) -> None:
    ...
    _save_settings(doc)
    record_manual_event(
        module="AU01", action="CORRECTION_AUTH_FAILED",
        record_type="ConstruControl Settings", record_id="ConstruControl Settings",
        reason=message, next_state={"failed_attempts": attempts}, origin="ADMIN_CORRECTION",
    )
...
def _derive(values: dict[str, Any]) -> dict[str, Any]:
    subtotal = flt(values.get("subtotal_hnl") if values.get("subtotal_hnl") not in (None, "") else values.get("amount_hnl"))
    total = round(subtotal + flt(values.get("tax_hnl")) - flt(values.get("withholding_hnl")) - flt(values.get("discount_hnl")), 2)
    if total < 0:
    ...
    if payment == "cancelled":
        ...
        payment, balance, approval, financial, operational = "draft", round(total - paid, 2), "draft", "pending", "pending"
    values.update({
        "subtotal_hnl": subtotal, "calculated_total_hnl": total, "amount_hnl": total,
        "paid_amount_hnl": paid, "balance_due_hnl": balance, "payment_status": payment,
        "professional_approval_status": approval,
        "approval_status": {"draft": "draft", "pending": "pending_approval", "approved": "approved", "rejected": "rejected"}.get(approval, "draft"),
        "approved_amount_hnl": total if approval == "approved" else 0.0,
        "financial_status": financial, "status": operational,
    })
    return values
...
def _prepare(name: str, operation: str, changes: Any, reason: str, evidence: str) -> dict[str, Any]:
    ...
    if operation == "annul_migrated":
        ...
        proposed.update({"paid_amount_hnl": 0, "payment_status": "cancelled", "professional_approval_status": "draft"})
        ...
        proposed["payment_status"] = str(requested.get("payment_status") or ("cancelled" if proposed["paid_amount_hnl"] <= 0 else "partially_paid"))
        proposed["professional_approval_status"] = "approved" if proposed["payment_status"] in {"approved", "partially_paid", "paid"} else "draft"
    ...
    before_amounts = expense_amounts(before.get("amount_hnl"), before.get("payment_status"), before.get("financial_status"), before.get("paid_amount_hnl"), before.get("balance_due_hnl"), before.get("professional_approval_status"))
    after_amounts = expense_amounts(proposed.get("amount_hnl"), proposed.get("payment_status"), proposed.get("financial_status"), proposed.get("paid_amount_hnl"), proposed.get("balance_due_hnl"), proposed.get("professional_approval_status"))
    impact = {
        "recognized_hnl": {"before": before_amounts[0], "after": after_amounts[0], "delta": round(after_amounts[0] - before_amounts[0], 2)},
        "paid_hnl": {"before": before_amounts[1], "after": after_amounts[1], "delta": round(after_amounts[1] - before_amounts[1], 2)},
        "pending_hnl": {"before": before_amounts[2], "after": after_amounts[2], "delta": round(after_amounts[2] - before_amounts[2], 2)},
        "funding_sources": sorted({str(v) for v in (before.get("funding_source"), proposed.get("funding_source")) if v}),
        "contracts": sorted({str(v) for v in (before.get("labor_contract"), proposed.get("labor_contract")) if v}),
        "projects": sorted({str(v) for v in (before.get("project"), proposed.get("project")) if v}),
    }
    payload = {"expense": name, "operation": operation, "reason": reason, "evidence": evidence, "before": before, "proposed": proposed, "impact": impact}
...
def configure_correction_pin(current_password: str, new_pin: str, enabled: int | str = 1) -> dict[str, Any]:
    ...
    record_manual_event(module="AU01", action="CORRECTION_PIN_CONFIGURED", record_type="ConstruControl Settings", record_id="ConstruControl Settings", reason="Administrator configuró o rotó la clave de corrección.", previous_state=before, next_state=after, origin="ADMIN_CORRECTION")
    return after
...
def authorize_correction(current_password: str, pin: str) -> dict[str, Any]:
    ...
    frappe.cache.set_value(_token_key(token), {"session_id": _session_id(), "authorization_id": authorization_id, "expires_at": expires_at}, expires_in_sec=_AUTH_TTL)
    record_manual_event(module="AU01", action="CORRECTION_AUTHORIZED", record_type="ConstruControl Settings", record_id="ConstruControl Settings", reason="Administrator abrió una sesión temporal de corrección.", next_state={"authorization_id": authorization_id, "expires_at": expires_at}, origin="ADMIN_CORRECTION", correlation_id=authorization_id)
    return {"token": token, "authorization_id": authorization_id, "expires_at": expires_at, "ttl_seconds": _AUTH_TTL}


@frappe.whitelist(methods=["POST"])
def preview_expense_correction(expense_name: str, operation: str, changes: Any, reason: str, evidence: str = "", authorization_token: str = "") -> dict[str, Any]:
    _require_token(authorization_token)
    return _prepare(str(expense_name or ""), operation, changes, reason, evidence)


@frappe.whitelist(methods=["POST"])
def execute_expense_correction(expense_name: str, operation: str, changes: Any, reason: str, evidence: str, preview_hash: str, authorization_token: str) -> dict[str, Any]:
    ...
    lock = frappe.cache.lock(f"construcontrol:admin-correction:expense:{payload['expense']}", timeout=120, blocking_timeout=5)
    ...
    try:
        ...
        with _correction_context():
            ...
            record_manual_event(
                module="MIG", action=f"ADMIN_{payload['operation'].upper()}",
                record_type="CC Expense Control", record_id=doc.name,
                project=str(after.get("project") or "") or None, reason=payload["reason"],
                previous_state=before,
                next_state={**after, "evidence": payload["evidence"], "impact": payload["impact"], "authorization_id": authorization["authorization_id"]},
                origin="ADMIN_CORRECTION", correlation_id=authorization["authorization_id"],
            )
        ...
        try:
            ...
            pass
    return {"expense": doc.name, "operation": payload["operation"], "authorization_id": authorization["authorization_id"], "before": before, "after": _snapshot(doc), "impact": payload["impact"]}


__all__ = ["authorize_correction", "configure_correction_pin", "execute_expense_correction", "get_security_status", "preview_expense_correction"]
