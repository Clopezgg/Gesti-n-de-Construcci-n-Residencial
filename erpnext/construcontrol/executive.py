...
def _label(value: Any) -> str:
    ...
    return _LABELS.get(
        raw,
        _LABELS.get(
            raw.lower(), raw.replace("_", " ").strip().capitalize() or "Sin clasificar"
        ),
    )
    ...
    return [
        {
            **dict(row),
            ...
            "record_type_label": str(
                row.get("module") or row.get("record_type") or "Registro"
            ),
        }
        for row in rows[:3]
    ]
...
def get_executive_dashboard(project: str | None = None) -> dict[str, Any]:
    ...
    summary = get_reporting_summary(
        date_from="2000-01-01", date_to=today(), project=project
    )
    ...
    payable_rows = [
        row
        for row in payables
        if str(row.get("payable_status") or "")
        not in {"paid", "cancelled", "reimbursed"}
    ]
        ...
        alerts.append(
            {
                ...
                "message": _("{0} material(es) requieren atención.").format(
                    len(low_stock)
                ),
        ...
        alerts.append(
            {
                ...
                "message": _(
                    "{0} hallazgo(s) de calidad requieren seguimiento."
                ).format(len(quality_issues)),
        ...
        alerts.append(
            {
                ...
                "message": _(
                    "El proyecto todavía no dispone de un cierre semanal cerrado."
                ),
    ...
    return {
        ...
        "financial": {
            ...
            "payable_balance_hnl": round(
                sum(flt(row.get("balance_due_hnl")) for row in payable_rows), 2
            ),
